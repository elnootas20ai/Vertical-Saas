...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-tiana-fondo-101.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/fix-tiana-fondo-101.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/fix-tiana-fondo-101.mjs ${apply ? '--apply' : ''}
`;

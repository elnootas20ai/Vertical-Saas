/**
 * Soft-delete «Boloñesa» de la carta Modomio (no es producto real).
 * Uso VPS: node scripts/remove-modomio-bolonesa.mjs --apply
 */
...
function isBolonesa(name) {
  const n = String(name || '')
    .trim()
    .toLowerCase()
    .normalize('NFD')
    .replace(/\p{M}/gu, '');
  return n === 'bolonesa' || n === 'boloñesa' || /^bolo.?esa$/i.test(n);
}
...
async function main() {
  ...
  for (const item of hits) {
    const next = {
      ...
      deletedAt: now,
      updatedAt: now,
      deletedReason: 'Retirada carta: Boloñesa no existe (pedido Uriel)',
    };

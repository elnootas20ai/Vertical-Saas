...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-badalona-aug-brand-tpv.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
echo '${scriptB64}' | base64 -d > scripts/diag-badalona-aug-brand-tpv.mjs
...
node scripts/diag-badalona-aug-brand-tpv.mjs
`;

...
const scriptPath = path.join(
  ...
  'diag-tiana-vs-badalona-stock-counts.mjs',
);
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
echo '${scriptB64}' | base64 -d > scripts/diag-tiana-vs-badalona-stock-counts.mjs
set -a; [ -f .env ] && . ./.env; set +a
node scripts/diag-tiana-vs-badalona-stock-counts.mjs
`;

...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-admin-tpv-stores.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/diag-admin-tpv-stores.mjs
...
`;
console.log('[remote-diag-admin-tpv-stores] SSH', `${user}@${host}`);

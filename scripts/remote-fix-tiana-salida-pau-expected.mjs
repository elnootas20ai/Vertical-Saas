...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-tiana-salida-pau-expected.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/fix-tiana-salida-pau-expected.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/fix-tiana-salida-pau-expected.mjs ${apply ? '--apply' : ''}
`;

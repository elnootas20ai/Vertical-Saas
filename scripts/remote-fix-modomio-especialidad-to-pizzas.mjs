#!/usr/bin/env node
/**
 * Remoto: Modomio Especialidad → Pizzas.
 *   node scripts/remote-fix-modomio-especialidad-to-pizzas.mjs
 *   node scripts/remote-fix-modomio-especialidad-to-pizzas.mjs --apply
 */
...
const scriptPath = path.join(
  ...
  'fix-modomio-especialidad-to-pizzas.mjs',
);
...
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/fix-modomio-especialidad-to-pizzas.mjs
node scripts/fix-modomio-especialidad-to-pizzas.mjs${APPLY ? ' --apply' : ''}
`;

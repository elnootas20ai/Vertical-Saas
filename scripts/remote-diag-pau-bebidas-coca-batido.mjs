...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-pau-bebidas-coca-batido.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const repo = values.REPO_PATH_ON_VPS?.trim();
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/diag-pau-bebidas-coca-batido.mjs
node scripts/diag-pau-bebidas-coca-batido.mjs
`;

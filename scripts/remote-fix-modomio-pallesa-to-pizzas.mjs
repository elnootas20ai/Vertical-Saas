#!/usr/bin/env node
/**
 * Remoto: mueve Pallesa → Pizzas en Modomio.
 *   node scripts/remote-fix-modomio-pallesa-to-pizzas.mjs
 *   node scripts/remote-fix-modomio-pallesa-to-pizzas.mjs --apply
 */
...
const scriptPath = path.join(
  ...
  'fix-modomio-pallesa-to-pizzas.mjs',
);
...
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/fix-modomio-pallesa-to-pizzas.mjs
node scripts/fix-modomio-pallesa-to-pizzas.mjs${APPLY ? ' --apply' : ''}
`;

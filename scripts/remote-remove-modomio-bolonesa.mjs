...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'remove-modomio-bolonesa.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/remove-modomio-bolonesa.mjs
node scripts/remove-modomio-bolonesa.mjs ${dry ? '' : '--apply'}
`;

...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'list-modomio-combos.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');
const repo = values.REPO_PATH_ON_VPS?.trim();
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/list-modomio-combos.mjs
node scripts/list-modomio-combos.mjs
`;

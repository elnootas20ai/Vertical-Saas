...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-tiana-push-tokens.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/diag-tiana-push-tokens.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/diag-tiana-push-tokens.mjs
echo '---'
(docker compose logs --since 45m app 2>/dev/null || true) | grep -E 'CEO_DAILY_DIGEST|pushService|nativePush|no tokens|APNs|Expo push' | tail -n 60
`;

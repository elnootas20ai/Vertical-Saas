...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-tiana-caja-txs.mjs');
...
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/diag-tiana-caja-txs.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/diag-tiana-caja-txs.mjs${sessionId ? ` '${sessionId}'` : ''}
`;

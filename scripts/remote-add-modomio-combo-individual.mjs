...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'add-modomio-combo-individual.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');
const repo = values.REPO_PATH_ON_VPS?.trim();
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/add-modomio-combo-individual.mjs
node scripts/add-modomio-combo-individual.mjs ${dry ? '' : '--apply'}
`;

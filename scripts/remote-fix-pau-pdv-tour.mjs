...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-pau-pdv-tour.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/fix-pau-pdv-tour.mjs
node scripts/fix-pau-pdv-tour.mjs ${apply ? '--apply' : ''}
`;

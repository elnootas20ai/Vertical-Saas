...
const scriptPath = path.join(
  ...
  'diag-joan-tejada-alert-detail.mjs',
);
const b64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/diag-joan-tejada-alert-detail.mjs
...
node scripts/diag-joan-tejada-alert-detail.mjs
echo "=== docker logs jotebe4 / Alerta ==="
docker logs deploy-app-1 2>&1 | grep -iE 'jotebe4|666152f5|\\[Alerta\\]' | tail -n 150 || true
`;
console.log('[remote-diag-joan-tejada-alert-detail] SSH', `${user}@${host}`);

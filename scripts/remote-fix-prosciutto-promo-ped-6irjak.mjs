...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-prosciutto-promo-ped-6irjak.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/fix-prosciutto-promo-ped-6irjak.mjs

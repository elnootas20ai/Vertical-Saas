...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'verify-pau-pdv-tour.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/verify-pau-pdv-tour.mjs
node scripts/verify-pau-pdv-tour.mjs
`;

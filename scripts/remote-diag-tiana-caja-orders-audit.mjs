...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-tiana-caja-orders-audit.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/diag-tiana-caja-orders-audit.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/diag-tiana-caja-orders-audit.mjs
`;

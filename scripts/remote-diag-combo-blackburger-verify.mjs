...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-combo-blackburger-verify.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const repo = values.REPO_PATH_ON_VPS?.trim();
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/diag-combo-blackburger-verify.mjs
node scripts/diag-combo-blackburger-verify.mjs
`;

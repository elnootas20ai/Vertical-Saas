...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-vb64cv-api-like.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
echo '${scriptB64}' | base64 -d > scripts/diag-vb64cv-api-like.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/diag-vb64cv-api-like.mjs
`;

...
const scriptPath = path.join(
  ...
  'fix-pau-cleanup-quick-attention-lost.mjs',
);
...
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/fix-pau-cleanup-quick-attention-lost.mjs
node scripts/fix-pau-cleanup-quick-attention-lost.mjs${APPLY ? ' --apply' : ''}
`;
console.log(APPLY ? '[remote] APPLY' : '[remote] dry — solo listar');
process.exit(
  sshRunScript(

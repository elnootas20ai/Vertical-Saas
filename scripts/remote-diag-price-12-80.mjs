...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-price-12-80.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/diag-price-12-80.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/diag-price-12-80.mjs
`;

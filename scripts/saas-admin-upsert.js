/**
 * Crea la cuenta SaaS (admin) en CouchDB `accounts` o resetea contraseña si ya existe.
 * Variables: COUCHDB_* + SAAS_LOGIN_EMAIL / SAAS_LOGIN_PASSWORD (+ opcionales SAAS_LOGIN_*).
 *
 * Carga env igual que el backend (.env.<NODE_ENV> luego .env).
 */
...
export async function upsertSaasLoginAdmin() {
  if (!BASE) {
  ...
  if (found?.docs?.length > 0) {
    ...
    console.log('✅ Contraseña actualizada (cuenta ya existía).');
    console.log(`   Email: ${LOGIN_EMAIL}`);
    console.log('   Entra con ese email y SAAS_LOGIN_PASSWORD del entorno.');
    return;
  }
  ...
  const doc = {
    ...
    onboardingData: { source: 'saas-admin-cli' },
    provider: 'email',
    permissions: {
  ...
  console.log(`   Email: ${LOGIN_EMAIL}`);
  console.log('   Usa ese email y SAAS_LOGIN_PASSWORD para iniciar sesión.');
}

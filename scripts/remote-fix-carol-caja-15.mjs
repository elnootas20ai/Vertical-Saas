...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-carol-caja-15.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/fix-carol-caja-15.mjs
set -a
[ -f .env ] && . ./.env
set +a
...
node scripts/fix-carol-caja-15.mjs ${apply ? '--apply' : ''}
`;

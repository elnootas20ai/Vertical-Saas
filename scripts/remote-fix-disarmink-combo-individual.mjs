...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-disarmink-combo-individual.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');
const repo = values.REPO_PATH_ON_VPS?.trim();
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/fix-disarmink-combo-individual.mjs
node scripts/fix-disarmink-combo-individual.mjs ${dry ? '' : '--apply'}
`;

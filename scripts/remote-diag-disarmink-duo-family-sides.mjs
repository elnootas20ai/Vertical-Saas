...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-disarmink-duo-family-sides.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const repo = values.REPO_PATH_ON_VPS?.trim();
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
echo '${b64}' | base64 -d > scripts/diag-disarmink-duo-family-sides.mjs
node scripts/diag-disarmink-duo-family-sides.mjs
`;
process.exit(
  sshRunScript(

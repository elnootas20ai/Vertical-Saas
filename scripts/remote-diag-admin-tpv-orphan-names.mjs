...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'diag-admin-tpv-orphan-names.mjs');
const b64 = fs.readFileSync(scriptPath).toString('base64');
const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${b64}' | base64 -d > scripts/diag-admin-tpv-orphan-names.mjs
...
node scripts/diag-admin-tpv-orphan-names.mjs
`;
console.log('[remote] SSH', `${user}@${host}`);

...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'rename-modomio-combo-pizzeria.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/rename-modomio-combo-pizzeria.mjs
node scripts/rename-modomio-combo-pizzeria.mjs ${dry ? '' : '--apply'}
`;

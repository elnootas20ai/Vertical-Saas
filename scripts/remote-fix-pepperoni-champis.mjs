...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fix-pepperoni-champis.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/fix-pepperoni-champis.mjs
node scripts/fix-pepperoni-champis.mjs ${dry ? '' : '--apply'}
`;

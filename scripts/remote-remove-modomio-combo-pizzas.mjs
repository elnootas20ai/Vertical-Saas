...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'remove-modomio-combo-pizzas.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/remove-modomio-combo-pizzas.mjs
node scripts/remove-modomio-combo-pizzas.mjs ${dry ? '' : '--apply'}
`;

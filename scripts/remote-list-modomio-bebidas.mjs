...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'list-modomio-bebidas.mjs');
const scriptB64 = fs.readFileSync(scriptPath).toString('base64');

const bash = `set -e
cd '${repo.replace(/'/g, `'\\''`)}'
mkdir -p scripts
echo '${scriptB64}' | base64 -d > scripts/list-modomio-bebidas.mjs
node scripts/list-modomio-bebidas.mjs
`;

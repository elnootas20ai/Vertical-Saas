...
(function (global) {
  ...
  const CONFIG = {
    ...
    container: attr('container', '#udar-stock'),
    lang: attr('lang', 'es'),
  ...
  function injectStyles(theme) {
    ...
    style.textContent = `
      ...
      .udar-widget *, .udar-widget *::before, .udar-widget *::after { box-sizing: border-box; }
      .udar-toolbar {
        display: flex;
        flex-wrap: wrap;
        gap: 10px;
      ...
      .udar-search:focus { border-color: ${th.accent}; }
      .udar-search::placeholder { color: ${th.textMuted}; }
      .udar-select {
        padding: 9px 12px;
        border-radius: 8px;
        border: 1px solid ${th.inputBorder};
      ...
      .udar-badge-available { background: ${th.badge_available}; color: ${th.badge_available_text}; }
      .udar-badge-reserved  { background: ${th.badge_reserved};  color: ${th.badge_reserved_text}; }
      .udar-badge-sold      { background: ${th.badge_sold};      color: ${th.badge_sold_text}; }
      .udar-card-meta {
        font-size: 12px;
        color: ${th.textMuted};
        display: flex;
      ...
      .udar-card-meta span { display: flex; align-items: center; gap: 3px; }
      .udar-price {
        font-size: 18px;
        font-weight: 700;
        color: ${th.accent};
      ...
      .udar-btn-detail:hover { opacity: .88; }
      .udar-pagination {
        display: flex;
        justify-content: center;
        align-items: center;
      ...
      .udar-page-btn:hover:not(:disabled) { background: ${th.accent}; color: ${th.accentText}; border-color: ${th.accent}; }
      .udar-page-btn:disabled { opacity: .4; cursor: default; }
      .udar-page-btn.active { background: ${th.btnActiveBg}; color: ${th.btnActiveText}; border-color: ${th.btnActiveBg}; }
      .udar-page-info { font-size: 13px; color: ${th.textMuted}; }
      .udar-status-msg {
        text-align: center;
        padding: 48px 24px;
        color: ${th.textMuted};
      ...
      .udar-spinner {
        ...
        border-radius: 50%;
        animation: udar-spin .7s linear infinite;
        margin-bottom: 12px;
      }
      @keyframes udar-spin { to { transform: rotate(360deg); } }
      .udar-footer {
        text-align: center;
        margin-top: 20px;
        font-size: 11px;
      ...
      .udar-footer a { color: ${th.accent}; text-decoration: none; }
      @media (max-width: 480px) {
        .udar-widget { padding: 16px; }
        .udar-grid { grid-template-columns: 1fr; }
      }
    `;
  ...
  function renderCard(v) {
    ...
    const statusClass = `udar-badge-${v.status || 'available'}`;
    const title = [v.brand, v.model].filter(Boolean).join(' ') || 'Vehículo';

    return `<div class="udar-card" data-id="${v._id}" role="article" tabindex="0" aria-label="${title}">
      ${imageUrl
        ? `<img class="udar-card-img" src="${imageUrl}" alt="${title}" loading="lazy" />`
        : `<div class="udar-card-img-placeholder" aria-hidden="true">🚗</div>`}
      <div class="udar-card-body">
        <div class="udar-card-header">
          <h3 class="udar-card-title">${title}</h3>
          <span class="udar-badge ${statusClass}">${getStatusLabel(v.status || 'available')}</span>
        </div>
        <div class="udar-card-meta">
        ...
        ${v.price ? `<div class="udar-price">${formatPrice(v.price)}</div>` : ''}
        <button class="udar-btn-detail" data-id="${v._id}">${t('viewDetails')}</button>
      </div>
    </div>`;
  }
  ...
  function render() {
    ...
    let html = `<div class="udar-widget">`;

    if (CONFIG.showFilters) {
      const brands = getBrands();
      html += `<div class="udar-toolbar" role="search">
        <input
          class="udar-search"
          type="search"
          ...
          aria-label="${t('search')}"
          id="udar-search-input"
        />
        ${CONFIG.status === 'all' ? `
        <select class="udar-select" id="udar-status-filter" aria-label="${t('filterStatus')}">
        ...
        ${brands.length > 1 ? `
        <select class="udar-select" id="udar-brand-filter" aria-label="${t('filterBrand')}">
    ...
    if (state.loading) {
      html += `<div class="udar-status-msg"><div class="udar-spinner"></div><br>${t('loading')}</div>`;
    } else if (state.error) {
      html += `<div class="udar-status-msg">⚠️ ${t('errorLoad')}</div>`;
    } else if (state.filtered.length === 0) {
      html += `<div class="udar-status-msg">🔍 ${t('noResults')}</div>`;
    } else {
      const page = paginate();
      html += `<div class="udar-grid" role="list">`;
      page.forEach((v) => { html += renderCard(v); });
      html += `</div>`;

      const total = totalPages();
      if (total > 1) {
        html += `<nav class="udar-pagination" aria-label="Paginación">
          <button class="udar-page-btn" id="udar-prev" ${state.page <= 1 ? 'disabled' : ''} aria-label="${t('prev')}">&lsaquo; ${t('prev')}</button>
          <span class="udar-page-info">${t('page')} ${state.page} ${t('of')} ${total}</span>
          <button class="udar-page-btn" id="udar-next" ${state.page >= total ? 'disabled' : ''} aria-label="${t('next')}">${t('next')} &rsaquo;</button>
        </nav>`;
      }
    }
  ...
  function attachEvents(container) {
    const searchInput = container.querySelector('#udar-search-input');
    if (searchInput) {
    ...
    const statusFilter = container.querySelector('#udar-status-filter');
    if (statusFilter) {
    ...
    const brandFilter = container.querySelector('#udar-brand-filter');
    if (brandFilter) {
    ...
    const prevBtn = container.querySelector('#udar-prev');
    if (prevBtn) {
    ...
    const nextBtn = container.querySelector('#udar-next');
    if (nextBtn) {
    ...
    container.querySelectorAll('.udar-btn-detail, .udar-card').forEach((el) => {
      el.addEventListener('click', (e) => {
        ...
        container.dispatchEvent(new CustomEvent('udar:vehicle-click', {
          bubbles: true,
          detail: { vehicle },
        }));
      });

...
export function CreateAppDialog({ open, onClose, onCreated }: Props) {
  ...
  const handleGenerateDesc = async () => {
    ...
    try {
      const result = await agentApi.enhanceDescription(description.trim(), sanitizedName || 'mi-app');
      if (result.enhanced) {
                ...
                <div className="flex items-center justify-between mt-1">
                  <div className="flex items-center gap-2">
                    <p className={cn(
                      ...
                      {t('minChars').replace('{min}', String(MIN_DESCRIPTION_LENGTH))}
                    </p>
                    {description.trim().length >= 10 && (
                      <button
                        ...
                        className={cn(
                          ...
                          generatingDesc
                            ? isDark ? 'bg-violet-900/30 text-violet-400' : 'bg-violet-100 text-violet-500'
                            : isDark

...
import {
  ...
  Building2, Users, Package, Warehouse, Settings,
} from 'lucide-react';
    ...
    return (
      <div className="px-2 py-3 border-t border-gray-200 dark:border-gray-700">
        <button
          ...
          title={`Arranque rápido: ${completionPct}%`}
    ...
    <div className="border-t border-gray-200 dark:border-gray-700">
      ...
      {expanded && (
        <div className="px-2 pb-3 space-y-0.5">
          {steps.map(step => (
            <StepRow key={step.id} step={step} onNavigate={() => navigate(step.route)} />
          ))}
          <button
            onClick={dismiss}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <p className={`text-xs font-medium leading-tight ${isCompleted ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'}`}>
            {step.label}
          </p>

...
const ROLES: {
  ...
  { id: 'Admin', dot: 'bg-slate-900', badgeBg: 'bg-slate-100', badgeText: 'text-slate-700', dotColor: '#111827', icon: <Shield className="w-3.5 h-3.5" />, desc: 'Control total de configuracion y permisos.', tier: 'pro' },
  { id: 'Gerente', dot: 'bg-blue-500', badgeBg: 'bg-blue-50', badgeText: 'text-blue-700', dotColor: '#3b82f6', icon: <Shield className="w-3.5 h-3.5" />, desc: 'Acceso completo a todos los modulos.', tier: 'standard' },
  { id: 'Comercial', dot: 'bg-emerald-500', badgeBg: 'bg-emerald-50', badgeText: 'text-emerald-700', dotColor: '#10b981', icon: <Star className="w-3.5 h-3.5" />, desc: 'Gestion de ventas, clientes y vehiculos.', tier: 'standard' },
  { id: 'Administracion', dot: 'bg-gray-400', badgeBg: 'bg-gray-100 dark:bg-gray-700', badgeText: 'text-gray-600 dark:text-gray-400', dotColor: '#9ca3af', icon: <User className="w-3.5 h-3.5" />, desc: 'Acceso a documentos, finanzas y clientes.', tier: 'normal' },
  { id: 'Taller', dot: 'bg-violet-500', badgeBg: 'bg-violet-50', badgeText: 'text-violet-700', dotColor: '#8b5cf6', icon: <Wrench className="w-3.5 h-3.5" />, desc: 'Gestion de reparaciones y mantenimiento.', tier: 'pro' },
  { id: 'Usuario', dot: 'bg-gray-400', badgeBg: 'bg-gray-100 dark:bg-gray-700', badgeText: 'text-gray-600 dark:text-gray-400', dotColor: '#9ca3af', icon: <User className="w-3.5 h-3.5" />, desc: 'Usuario basico con permisos personalizables.', tier: 'normal' },
];
...
function getRoleTier(roleId: string) {
  if (roleId === 'Admin' || roleId === 'Taller') return 'pro' as const;
  if (roleId === 'Administracion' || roleId === 'Usuario') return 'normal' as const;
...
function getRoleConfig(role: RoleDefinition) {
  const fallback = {
    ...
    desc: role.description || 'Rol personalizado.', tier: 'standard' as const,
  };
...
export function InviteUserModal({ onClose, onInvite, roles, workCenters, businesses, currentBusinessId }: InviteUserModalProps) {
  ...
  const roleOptions = roles?.length ? roles : ROLES.map((item) => ({ id: item.id, description: item.desc, permissions: [], users: 0 }));
  ...
  function validateStep2(): Record<string, string> {
    const e: Record<string, string> = {};
    if (!position.trim()) e.position = 'El puesto/funcion es obligatorio';
    if (!contractType) e.contractType = 'Selecciona un tipo de contrato';
    if (!role) e.role = t('team.inviteModal.roleRequired', 'Selecciona un rol');
    return e;
  }
  ...
  async function handleSubmit() {
    ...
    if (Object.keys(errs).length > 0) {
      setErrors(errs);
      setTouched({ position: true, contractType: true, role: true });
      return;
    }
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              {step === 1 ? (
                ...
                <>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                      Puesto / Funcion <span className="text-red-400">*</span>
                    </label>
                    <div className="relative">
                      ...
                      <input type="text" value={position}
                        ...
                        placeholder="Ej: Comercial, Mecanico, Administrativo..."
                        className={`w-full pl-10 pr-4 py-2.5 border-2 rounded-xl text-sm outline-none transition-colors text-gray-900 dark:text-gray-100 ${
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                      Rol en la plataforma <span className="text-red-400">*</span>
                    </label>
                    <RoleDropdown value={role}
                      ...
                      selectRolePlaceholder={t('team.inviteModal.selectRole', 'Selecciona un rol')} />
                  </div>

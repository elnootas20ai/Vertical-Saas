  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4 space-y-2 text-sm">
        ...
        <div className="flex justify-between"><span className="text-gray-500">Fondo de apertura</span><span className="font-semibold">{session.initialCashAmount.toFixed(2)}€</span></div>
        ...
        <div className="flex justify-between"><span className="text-red-600">− Devoluciones efectivo</span><span className="font-semibold text-red-700">{cashReturns.toFixed(2)}€</span></div>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-2 flex justify-between">
          ...
          <span className="font-bold text-emerald-700 dark:text-emerald-400">{(session.expectedCash ?? 0).toFixed(2)}€</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-700 dark:text-gray-300 font-medium">Efectivo contado</span>
          <span className="font-bold text-gray-900 dark:text-gray-100">{(session.finalCashAmount ?? 0).toFixed(2)}€</span>
        </div>
        <div className={`flex justify-between p-2 rounded-lg ${session.difference === 0 ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
          ...
          <span className={`font-bold ${session.difference === 0 ? 'text-green-700' : 'text-red-700'}`}>
            {(session.difference ?? 0) >= 0 ? '+' : ''}{(session.difference ?? 0).toFixed(2)}€
          </span>
        </div>
      </div>
      ...
      {session.transactions.some((t) => t.type === 'cash_in' || t.type === 'cash_out' || t.type === 'return') && (
        <div>
          ...
          <div className="space-y-1">
            {session.incidents.map((inc) => (
              <div key={inc.id} className="flex items-center justify-between text-xs p-2 bg-gray-50 dark:bg-gray-900 rounded-lg">
                ...
                {inc.amount != null && <span className="font-semibold shrink-0 ml-2">{inc.amount.toFixed(2)}€</span>}
              </div>
            ))}
          </div>
        </div>

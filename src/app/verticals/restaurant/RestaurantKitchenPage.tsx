...
export function RestaurantKitchenPage() {
  ...
  // Tiempo real: eventos de sala por SSE + polling de respaldo + refresh al volver a la pestaña.
  ...
  useEffect(() => {
    if (!userId || !isRestaurant) return;
    const iv = setInterval(loadOrders, 20_000);
    return () => clearInterval(iv);
  ...
  // Tickets por columna (se filtra por la empresa activa; multiempresa no mezcla salas)

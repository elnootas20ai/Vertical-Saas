...
import { Check, Loader2, Move, Save } from 'lucide-react';
import {
...
export function RestaurantSalaFloorEditor({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 p-3 sm:p-4">
      ...
      <div className="flex flex-col gap-2 rounded-xl border border-stone-200 bg-white p-2 dark:border-stone-700 dark:bg-stone-900 sm:flex-row sm:flex-wrap sm:items-center">
        {sortedRooms.length > 1 ? (
          <div
            className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5"
            style={{ scrollbarWidth: 'none' }}
          >
            <span className="mr-0.5 px-1 text-[11px] font-bold uppercase tracking-wide text-stone-500">
              Zona
            </span>
            {sortedRooms.map((room) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-9 shrink-0 rounded-lg border px-3 text-xs font-semibold transition-colors ${
                    active
          ...
          <p className="min-w-0 flex-1 truncate px-1 text-sm font-semibold text-stone-800 dark:text-stone-100">
            {activeRoom?.name}
          </p>
        )}

        <div className="flex flex-wrap items-center gap-1.5 sm:border-l sm:border-stone-200 sm:pl-2 dark:sm:border-stone-700">
          <span className="mr-0.5 px-1 text-[11px] font-bold uppercase tracking-wide text-stone-500">
            Fondo
          </span>

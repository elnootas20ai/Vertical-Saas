...
import {
  ...
  updateRestaurantRegisterSession,
...
import {
  ...
  ShieldCheck, ArrowLeft, History, UtensilsCrossed, Radio, Receipt,
} from 'lucide-react';
...
function ValidationModal({ session, salaOrders, onValidate, onReject, onCancel }: {
  session: TpvRegisterSession;
  salaOrders: DiningOrder[];
  onValidate: (notes: string) => void;
  onReject: (notes: string) => void;
  onCancel: () => void;
}) {
  const [notes, setNotes] = useState('');
  const salaTotal = salaOrders.reduce((s, o) => s + Number(o.total || 0), 0);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col min-h-0" style={{ maxHeight: '96vh' }}>
        <div className="flex-shrink-0 p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 mt-1">
            {session.pointOfSaleName} · {salaOrders.length} cuenta{salaOrders.length !== 1 ? 's' : ''} de sala ({salaTotal.toFixed(2)}€)
          </p>
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-4">
          ...
          {salaOrders.length > 0 && (
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-4">
              <h3 className="text-sm font-bold mb-2 flex items-center gap-2"><UtensilsCrossed className="w-4 h-4" /> Cuentas de sala en el turno</h3>
              <div className="max-h-40 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700 text-xs">
                {salaOrders.map((o) => (
                  <div key={o._id} className="flex justify-between py-2">
                    <span>Mesa {o.tableNumber} · {o.clientName || o.tableName || '—'}</span>
        ...
        <div className="flex-shrink-0 p-6 border-t border-gray-200 dark:border-gray-700 flex gap-3">
          ...
          <button onClick={() => onReject(notes)} className="px-5 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold text-sm transition-colors">Rechazar</button>
...
export function RestaurantCajaPage() {
  ...
  const validationSalaOrders = useMemo(() => {
    if (!validatingSession) return [];
    return salaOrdersInRange.filter((o) => orderInRegisterSession(o, validatingSession));
  }, [validatingSession, salaOrdersInRange]);
    ...
    try {
      const updatedList = await Promise.all(
        emptyPendingClosures.map((s) =>
          updateRestaurantRegisterSession(dataUserId, {
            ...s,
  ...
  const handleValidate = async (notes: string) => {
    ...
    try {
      const updated = await updateRestaurantRegisterSession(dataUserId, {
        ...validatingSession,
        closingValidatedBy: user?.name || user?.email || 'Gerente',
        closingValidatedAt: new Date().toISOString(),
        closingValidationStatus: 'validated',
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"
            >
        ...
        <section>
          ...
          >
            <span className="flex items-center gap-2">
              ...
              3 · Cuentas de sala del día (opcional)
            </span>
            <span className="text-xs text-gray-400 font-normal">Cobros desde mesas</span>
          </button>
            ...
            <div className="mt-3 pl-1">
              <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold flex items-center gap-2"><UtensilsCrossed className="w-4 h-4" /> Cuentas cobradas hoy</h3>
                  <button type="button" onClick={() => void loadSalaOrders()} disabled={loadingOrders} className="text-xs text-indigo-600 font-semibold">
                    {loadingOrders ? 'Cargando…' : 'Actualizar'}
                  </button>
                </div>
                {salaOrdersInRange.length === 0 ? (
                  <p className="text-xs text-gray-400 text-center py-6">Sin cuentas de sala este día</p>
                ) : (
                  <div className="max-h-48 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700 text-xs">
                    {salaOrdersInRange.map((o) => (
                      <div key={o._id} className="flex justify-between py-2">
                        <span>Mesa {o.tableNumber} · {new Date(o.paidAt || o.createdAt || '').toLocaleTimeString('es-ES', { timeStyle: 'short' })}</span>
                        <span className="font-bold">{Number(o.total || 0).toFixed(2)}€</span>
                      </div>
                    ))}
                  </div>
      ...
      {validatingSession && (
        <ValidationModal
          ...
          salaOrders={validationSalaOrders}

...
type Props = {
  ...
  rootMargin?: string;
  className?: string;
  /** Compatibilidad al sustituir DashboardLazyPanel; Delivery no persiste cierres. */
  storageKey?: string;
  defaultOpen?: boolean;
};
...
function DeliverySectionSkeleton({
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 h-16 rounded-xl bg-stone-50 dark:bg-stone-950/50" />
    </section>
  );
}

/**
 * Panel progresivo del dashboard Delivery: aparece solo al acercarse al viewport,
 * reserva altura y no exige ninguna acción de abrir/cerrar.
 */
export function DeliveryProgressiveSection({
  title,
  hint,
  icon,
  children,
  ...
  return (
    <div
      ...
      className={className}
      style={{ minHeight }}
      data-delivery-progressive-section={title}
    >
      {shouldMount ? (
        <div
        ...
        <DeliverySectionSkeleton title={title} hint={hint} icon={icon} />
      )}
    </div>
  );
}

...
export function ScrapyardEnvironment() {
  ...
  const persistEntries = async (entries: Record<string, unknown>[]) => {
    ...
    const tipoResiduo = entryStr(e, 'tipoResiduo');
    if (!tipoResiduo) return null;
    return {
      tipoResiduo,
      cantidad: entryNum(e, 'cantidad', 'quantity', 'qty'),
      unidad: entryStr(e, 'unidad', 'unit') || 'Litros',
      gestorAutorizado: entryStr(e, 'gestorAutorizado') || '',
      numDocumento: entryStr(e, 'numDocumento') || '',
      fechaRecogida: entryStr(e, 'fechaRecogida') || new Date().toISOString().slice(0, 10),
      estado: entryStr(e, 'estado', 'status') || 'Almacenado',
    };
    });
    if (created > 0) {
      await loadData();
      toast.success(`${created} registro ambiental creado(s)`);
    } else {

...
export function NightclubGuestlist() {
  ...
  const persistEntries = async (entries: Record<string, unknown>[]) => {
    ...
    return {
      nombre,
      promotor: entryStr(e, 'promotor') || '',
      evento: entryStr(e, 'evento') || '',
      tipo: entryStr(e, 'tipo', 'type') || 'lista',
      horaLlegada: entryStr(e, 'horaLlegada') || '',
      estado: entryStr(e, 'estado', 'status') || 'en_lista',
    };
    });
    if (created > 0) {
      await loadData();
      toast.success(`${created} invitado creado(s)`);
    } else {

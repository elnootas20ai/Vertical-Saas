...
export function GymRoutines() {
  ...
  const persistEntries = async (entries: Record<string, unknown>[]) => {
    ...
    return {
      nombre,
      tipo: entryStr(e, 'tipo', 'type') || 'mixta',
      duracion: entryNum(e, 'duracion', 'duration'),
      nivel: entryStr(e, 'nivel') || 'principiante',
      ejercicios: [{ ...EMPTY_EXERCISE }],
      asignados: [],
    };
    });
    if (created > 0) {
      await loadData();
      toast.success(`${created} rutina creado creado(s)`);
    } else {

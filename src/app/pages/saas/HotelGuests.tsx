...
export function HotelGuests() {
  ...
  const persistEntries = async (entries: Record<string, unknown>[]) => {
    ...
    return {
      ...
      document: entryStr(e, 'document') || '',
      nationality: entryStr(e, 'nationality') || '',
      phone: entryStr(e, 'phone') || '',
      email: entryStr(e, 'email') || '',
      previousStays: entryNum(e, 'previousStays'),
      preferences: entryStr(e, 'preferences') || '',
      vip: false,
    };
    });
    if (created > 0) {
      await loadData();
      toast.success(`${created} huésped creado(s)`);
    } else {

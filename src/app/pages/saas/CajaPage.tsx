...
import {
  ...
  Truck, MapPin,
  ArrowLeft, Plug, History, ShoppingBag, Radio, Lock, RotateCcw,
} from 'lucide-react';
import {
          ...
          {showReason && canReadjust ? (
            <div className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50/80 dark:bg-amber-950/30 p-3 space-y-2">
              ...
              <textarea
                ...
                placeholder="Ej: Error en el conteo de efectivo, apps mal declaradas…"
                autoFocus
              />
...
export function CajaPage() {
  ...
  const handleReadjustClosing = useCallback(async (reason: string) => {
    ...
    try {
      ...
      toast.success(
        `Caja reajustada: ${reopened.pointOfSaleName || 'tienda'}. Continúa el turno en el TPV.`,
      );
    } catch (err) {

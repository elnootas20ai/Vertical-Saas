...
type ActivityKind = 'prescription' | 'inventory' | 'guard';
...
function kindFromPharmacyType(type: string): ActivityKind {
  if (type.includes('prescription')) return 'prescription';
  if (type.includes('medication') || type.includes('inventory')) return 'inventory';
...
export function PharmacyDashboard({ onSelectGeneral }: PharmacyDashboardProps) {
  const { user } = useAuth();
  const dashApi = useMemo(() => createVerticalDashboardApi('pharmacy'), []);
  ...
  const activitiesFromApi = useMemo((): ActivityItem[] => {
    ...
    return raw.map((a) => ({
      id: a.id,
      kind: kindFromPharmacyType(a.type || ''),
  ...
  const monthSummary = useMemo(() => {
    ...
    return {
      recetas: cnt?.prescriptions ?? 0,
      ventas: `€${(cnt?.sales ?? 0).toLocaleString('es-ES')}`,
      pedidos: cnt?.suppliers ?? 0,
      alertasStock: cnt?.inventory ?? 0,
    };
  }, [dashData]);
  ...
  const activityIcon = (kind: ActivityKind) => {
    switch (kind) {
      ...
      case 'inventory':
        return <PackageX className="h-4 w-4 text-amber-600 dark:text-amber-400" />;
      default:
        return <UserRound className="h-4 w-4 text-violet-600 dark:text-violet-400" />;
    }
  };
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {(c?.prescriptions ?? 0).toLocaleString('es-ES')}
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Recetas dispensadas hoy</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            <div className="mb-3 flex items-center justify-between">
              <div className="rounded-xl bg-blue-50 p-2.5 dark:bg-blue-900/30">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              €{(c?.sales ?? 0).toLocaleString('es-ES')}
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Ventas hoy</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            <div className="mb-3 flex items-center justify-between">
              <div className="rounded-xl bg-blue-50 p-2.5 dark:bg-blue-900/30">
                <PackageX className="h-5 w-5 text-blue-600 dark:text-blue-400" />
              </div>
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {(c?.inventory ?? 0).toLocaleString('es-ES')}
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Productos bajo stock</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            <div className="mb-3 flex items-center justify-between">
              <div className="rounded-xl bg-blue-50 p-2.5 dark:bg-blue-900/30">
            ...
            <p className="text-lg font-bold leading-tight text-gray-900 dark:text-white">
              {(c?.guardShifts ?? 0) > 0 ? String(c?.guardShifts) : '—'}
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Próx. guardia</p>
          </div>
        </div>
              ...
              <div className="flex flex-wrap gap-1">
                {(
                  [
                    ...
                    { key: 'prescription' as const, label: 'Recetas' },
                    { key: 'inventory' as const, label: 'Inventario' },
                    { key: 'guard' as const, label: 'Guardias' },
                  ] as const
                ).map(({ key, label }) => (
                  <button
                    key={key}
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            ...
            <ul className="space-y-4">
              <li className="flex justify-between border-b border-gray-100 pb-3 dark:border-gray-700">
                <span className="text-sm text-gray-500 dark:text-gray-400">Recetas dispensadas</span>
                <span className="font-semibold text-gray-900 dark:text-white">
                  {monthSummary.recetas.toLocaleString('es-ES')}
                </span>
              </li>
              <li className="flex justify-between border-b border-gray-100 pb-3 dark:border-gray-700">
                <span className="text-sm text-gray-500 dark:text-gray-400">Ventas totales</span>
                <span className="font-semibold text-gray-900 dark:text-white">{monthSummary.ventas}</span>
              </li>
              <li className="flex justify-between border-b border-gray-100 pb-3 dark:border-gray-700">
                <span className="text-sm text-gray-500 dark:text-gray-400">Pedidos a proveedores</span>
                <span className="font-semibold text-gray-900 dark:text-white">{monthSummary.pedidos}</span>
              </li>
              <li className="flex justify-between">
                <span className="text-sm text-gray-500 dark:text-gray-400">Alertas de stock resueltas</span>
                <span className="font-semibold text-amber-600 dark:text-amber-400">{monthSummary.alertasStock}</span>
              </li>
            </ul>
          </div>
        </div>

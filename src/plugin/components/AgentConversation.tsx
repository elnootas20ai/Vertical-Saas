...
import type { ConnectionStatus } from '../hooks/usePlugin';
...
function ErrorMessage({ message }: { message: ChatMessage }) {
  ...
  const displayContent = message.content.replace(/^Error:\s*/i, '');
  const errorDetail = message.errorDetail || '';
  const hasDetail = errorDetail && errorDetail !== displayContent;
  ...
  return (
    <div className="flex items-start gap-2.5 animate-in fade-in slide-in-from-bottom-2 duration-200 min-w-0">
      <div className={cn(
        ...
        <AlertTriangle className={cn('size-3.5', isDark ? 'text-red-400' : 'text-red-500')} />
      </div>
      <div className="flex-1 min-w-0 overflow-hidden">
        {message.thinking && (
          <ThinkingBlock thinking={message.thinking} defaultCollapsed />
        )}
        ...
        >
            ...
            <div className="mt-2">
              <button
                ...
                className={cn(
                  ...
                  isDark ? 'text-red-400/70 hover:text-red-300' : 'text-red-500 hover:text-red-700',
                )}
              >
              ...
              {expanded && (
                <pre
                  className={cn(
                    'mt-1.5 p-2 rounded-lg text-[11px] leading-relaxed overflow-x-auto',
                    isDark
                      ? 'bg-red-950/60 text-red-300/80 border border-red-800/30'
          ...
          <p className={cn('text-[9px] mt-1.5', isDark ? 'text-red-600/60' : 'text-red-400')}>
...
function AssistantMessage({ message, isStreaming, isLast }: { message: ChatMessage; isStreaming: boolean; isLast: boolean }) {
  ...
  if (message.isError || (!showCursor && message.content.startsWith('Error:'))) {
...
export function AgentConversation({
  ...
  return (
    <div className="flex flex-col h-full w-full min-w-0 overflow-hidden">
          ...
          >
            ...
            <div className="flex-1 min-w-0 text-left">
              <div className="flex items-center gap-1.5">
                ...
                <span className={cn('size-1.5 rounded-full shrink-0 transition-colors',
                  isBusy ? 'bg-violet-400 animate-pulse'
                    : connectionStatus === 'reconnecting' ? 'bg-amber-400 animate-pulse'
                    : 'bg-emerald-400',
                )} title={connectionStatus === 'reconnecting' ? 'Reconectando...' : connectionStatus === 'connected' ? 'Conectado' : ''} />
                {agent.model && (
      ...
      {/* Agent error bar - only show real errors, not connection hiccups */}
      {agentError && !isBusy && (
        <div className={cn('flex items-center justify-between gap-2 px-3 py-1.5 text-[10px] font-medium shrink-0 border-b',
          isDark ? 'bg-red-950/30 text-red-400 border-red-900/30' : 'bg-red-50 text-red-600 border-red-100')}>
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <AlertTriangle className="size-3 shrink-0" />
            <span className="truncate">{agentError}</span>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            {onForceReset && (
              <button onClick={onForceReset}
                className={cn('px-2 py-0.5 rounded text-[9px] font-bold transition-colors',
            ...
            {onDismissError && (
              <button onClick={onDismissError}
                className={cn('px-1 py-0.5 rounded transition-colors', isDark ? 'hover:bg-red-900/30' : 'hover:bg-red-100')}>
                <X className="size-3" />
              </button>
            )}
          </div>
      ...
      >
        ...
        >
          ...
          {(attachedFiles.length > 0 || attachedElements.length > 0 || (ticketLink && onGoToTicket)) && (
            <div className="flex flex-wrap gap-1 pb-0.5">
              {ticketLink && onGoToTicket && (
                <button
                  onClick={(e) => { e.stopPropagation(); onGoToTicket(ticketLink.ticketId); }}
                  className={cn(
                    'group/ticket inline-flex items-center gap-1.5 pl-1.5 pr-2 py-0.5 rounded-md text-[10px] font-medium transition-all duration-200 max-w-[220px] cursor-pointer',
                    isDark
                      ? 'bg-violet-500/10 text-violet-300 border border-violet-500/25 hover:bg-violet-500/20 hover:border-violet-400/40 hover:shadow-[0_0_8px_rgba(139,92,246,0.15)]'
                      : 'bg-violet-50 text-violet-700 border border-violet-200 hover:bg-violet-100 hover:border-violet-300 hover:shadow-sm',
                  )}
                  title={`Ir al ticket: ${ticketLink.ticketTitle}`}
                >
                  <Bookmark className={cn('size-2.5 shrink-0 transition-transform duration-200 group-hover/ticket:scale-110',
                    isDark ? 'text-violet-400/70' : 'text-violet-500')} />
                  <span className="truncate">{ticketLink.ticketTitle}</span>
                  <ExternalLink className={cn('size-2.5 shrink-0 opacity-0 -ml-0.5 group-hover/ticket:opacity-70 group-hover/ticket:ml-0 transition-all duration-200',
          ...
          <div className="flex items-center justify-between">
            ...
            {isBusy && onStop ? (
              <button
                onClick={onStop}
                className="px-3 py-1 rounded-lg text-[11px] font-medium flex items-center gap-1.5 transition-all bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/30"
              >
                <X className="size-3" />
                Stop
              </button>

...
import {
  ...
  GanttChart, ChevronLeft, ZoomIn, ZoomOut, Check, ListTodo,
} from 'lucide-react';
import type {
...
type KanbanView = 'board' | 'ai_search';
...
function TaskEntryRow({
  ...
  const [done, setDone] = useState(entry.hours < 0);
  ...
  const adjustHours = async (delta: number) => {
    const next = Math.max(0, Math.round((hours + delta) * 10) / 10);
    setHours(next);
    await agentApi.kanbanUpdateTimeEntry(ticketId, entry.id, { hours: next, description: desc, date });
    onUpdate();
  };
  ...
  const toggleDone = async () => {
    const nextDone = !done;
    setDone(nextDone);
    const nextHours = nextDone ? -Math.abs(hours || 1) : Math.abs(hours || 1);
    setHours(nextHours);
    await agentApi.kanbanUpdateTimeEntry(ticketId, entry.id, { hours: nextHours, description: desc, date });
    onUpdate();
  };

  const isAiEntry = entry.userId === 'ai-agent';
  const priorityMatch = entry.description?.match(/^\[(critical|high|medium|low|lowest)\]\s*/i);
  const priority = priorityMatch ? priorityMatch[1].toLowerCase() : null;
  const cleanDesc = priorityMatch ? entry.description!.slice(priorityMatch[0].length) : (entry.description || entry.userName);
  const [descTitle, ...descRest] = cleanDesc.split('\n');
  const descBody = descRest.join('\n').trim();

  const priorityColors: Record<string, string> = {
    critical: isDark ? 'bg-red-900/40 text-red-300 border-red-800/40' : 'bg-red-50 text-red-600 border-red-200',
    high: isDark ? 'bg-orange-900/40 text-orange-300 border-orange-800/40' : 'bg-orange-50 text-orange-600 border-orange-200',
    medium: isDark ? 'bg-amber-900/40 text-amber-300 border-amber-800/40' : 'bg-amber-50 text-amber-600 border-amber-200',
    low: isDark ? 'bg-sky-900/40 text-sky-300 border-sky-800/40' : 'bg-sky-50 text-sky-600 border-sky-200',
    lowest: isDark ? 'bg-zinc-800 text-zinc-400 border-zinc-700' : 'bg-gray-100 text-gray-500 border-gray-200',
  };

  if (editing) {
    return (
      <div className={cn('rounded-lg border p-2.5 space-y-2', isDark ? 'border-violet-700/50 bg-zinc-900' : 'border-violet-200 bg-gray-50')}>
        <AutoResizeTextarea
          className={cn(inputClass, 'w-full text-[11px] font-medium')}
        ...
        <div className="flex items-center gap-2">
          <input
            className={cn(inputClass, 'w-24 text-[10px]')}
            type="date"
            value={date}
            onChange={(e) => setDate(e.target.value)}
          />
          <div className="flex-1" />
          <button onClick={handleDelete} className={cn('p-1 rounded transition-colors', isDark ? 'text-red-400 hover:bg-red-900/20' : 'text-red-500 hover:bg-red-50')}>
            <Trash2 className="size-3" />
          </button>
          <button onClick={() => { setEditing(false); setDesc(entry.description || ''); setDate(entry.date || ''); setHours(entry.hours); }} className={cn('p-1 rounded', isDark ? 'text-zinc-400' : 'text-gray-500')}>
            <X className="size-3" />
          </button>
          <button onClick={handleSave} className="px-2.5 py-1 rounded text-[10px] font-medium bg-violet-600 text-white hover:bg-violet-500 transition-colors">
            Guardar
          </button>
        </div>
      </div>
  ...
  const isDoneEntry = hours < 0 || done;

  return (
    <div className={cn(
      'group flex items-start gap-2 rounded-lg border px-2.5 py-2 transition-all',
      isDoneEntry
        ? isDark ? 'bg-emerald-950/10 border-emerald-900/20' : 'bg-emerald-50/50 border-emerald-200/60'
        : isAiEntry
          ? isDark ? 'bg-violet-950/15 border-violet-800/25 hover:border-violet-700/40' : 'bg-violet-50/40 border-violet-200 hover:border-violet-300'
          : isDark ? 'bg-zinc-900/50 border-zinc-800 hover:border-zinc-700' : 'bg-white border-gray-200 hover:border-gray-300',
    )}>
      {/* Checkbox */}
      <button onClick={toggleDone} className={cn(
        'mt-0.5 size-4 rounded border-[1.5px] flex items-center justify-center shrink-0 transition-all',
        isDoneEntry
          ? isDark ? 'bg-emerald-600/30 border-emerald-500 text-emerald-400' : 'bg-emerald-500 border-emerald-500 text-white'
          : isDark ? 'border-zinc-600 hover:border-violet-500' : 'border-gray-300 hover:border-violet-400',
      )}>
        {isDoneEntry && <Check className="size-2.5" />}
      </button>
      ...
      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setEditing(true)}>
        <div className="flex items-center gap-1.5 flex-wrap">
          {priority && (
            <span className={cn('text-[7px] font-bold uppercase px-1 py-[1px] rounded border leading-none', priorityColors[priority])}>
              {priority}
            </span>
          )}
          {isAiEntry && <Sparkles className={cn('size-2.5 shrink-0', isDark ? 'text-violet-400' : 'text-violet-500')} />}
          <span className={cn(
            'text-[11px] font-medium leading-snug',
            isDoneEntry ? 'line-through opacity-50' : '',
            isDark ? 'text-zinc-200' : 'text-gray-800',
          )}>{descTitle}</span>
        </div>
        {descBody && (
          <p className={cn('text-[9px] mt-0.5 leading-relaxed line-clamp-2', isDoneEntry ? 'opacity-30' : '', isDark ? 'text-zinc-500' : 'text-gray-500')}>{descBody}</p>
        )}
        {date && (
          <span className={cn('text-[8px] mt-0.5 inline-block', isDark ? 'text-zinc-600' : 'text-gray-400')}>{date}</span>
        )}
      </div>

      {/* Hours +/- */}
      <div className={cn(
        'flex items-center gap-0 shrink-0 rounded-md border',
        isDark ? 'border-zinc-700 bg-zinc-800/50' : 'border-gray-200 bg-gray-50',
      )}>
        <button
          onClick={() => adjustHours(-0.5)}
          className={cn('px-1 py-0.5 transition-colors rounded-l-md', isDark ? 'hover:bg-zinc-700 text-zinc-400' : 'hover:bg-gray-200 text-gray-500')}
        >
          <span className="text-[10px] font-bold leading-none">−</span>
        </button>
        <span className={cn('text-[10px] font-mono font-bold px-1 min-w-[28px] text-center tabular-nums', isDark ? 'text-zinc-200' : 'text-gray-700')}>
          {Math.abs(hours).toFixed(1)}h
        </span>
        <button
          onClick={() => adjustHours(0.5)}
          className={cn('px-1 py-0.5 transition-colors rounded-r-md', isDark ? 'hover:bg-zinc-700 text-zinc-400' : 'hover:bg-gray-200 text-gray-500')}
        >
          <span className="text-[10px] font-bold leading-none">+</span>
        </button>
      </div>
...
type TicketFilter = 'all' | 'pending' | 'done' | 'ai';

function TicketsSection({
  ticket, isDark, inputClass, totalHours,
  timeHours, setTimeHours, timeDesc, setTimeDesc, handleAddTime, onUpdate,
}: {
  ticket: KanbanTicket;
  isDark: boolean;
  inputClass: string;
  ...
  timeDesc: string;
  setTimeDesc: (v: string) => void;
  handleAddTime: () => void;
  onUpdate: (partial: Partial<KanbanTicket>) => void;
}) {
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState<TicketFilter>('all');
  const [addingNew, setAddingNew] = useState(false);

  const entries = ticket.timelog || [];

  const filtered = useMemo(() => {
    let list = entries;
    if (filter === 'done') list = list.filter(e => e.hours < 0);
    else if (filter === 'pending') list = list.filter(e => e.hours >= 0);
    else if (filter === 'ai') list = list.filter(e => e.userId === 'ai-agent');
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(e => (e.description || '').toLowerCase().includes(q) || (e.userName || '').toLowerCase().includes(q));
    }
    return list;
  }, [entries, filter, search]);

  const countDone = entries.filter(e => e.hours < 0).length;
  const countPending = entries.filter(e => e.hours >= 0).length;
  const countAi = entries.filter(e => e.userId === 'ai-agent').length;

  const filters: { k: TicketFilter; label: string; count: number }[] = [
    { k: 'all', label: 'Todo', count: entries.length },
    { k: 'pending', label: 'Pendiente', count: countPending },
    { k: 'done', label: 'Hecho', count: countDone },
    { k: 'ai', label: 'IA', count: countAi },
  ];
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1.5">
        <div className={cn('flex items-center gap-1.5 rounded-lg border px-2 py-1', isDark ? 'border-zinc-800 bg-zinc-900/50' : 'border-gray-200 bg-gray-50')}>
          <Search className={cn('size-3 shrink-0', isDark ? 'text-zinc-500' : 'text-gray-400')} />
          <input
            className={cn('flex-1 bg-transparent border-none outline-none text-[10px]', isDark ? 'text-zinc-200 placeholder-zinc-600' : 'text-gray-800 placeholder-gray-400')}
            placeholder="Buscar tickets..."
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
          {search && (
            <button onClick={() => setSearch('')} className={cn('p-0.5', isDark ? 'text-zinc-500' : 'text-gray-400')}>
          ...
          {filters.map(f => (
            <button
              ...
              className={cn(
                ...
                filter === f.k
                  ? isDark ? 'bg-violet-600/30 text-violet-300 border border-violet-600/40' : 'bg-violet-100 text-violet-700 border border-violet-200'
                  : isDark ? 'text-zinc-500 hover:text-zinc-300 border border-transparent' : 'text-gray-500 hover:text-gray-700 border border-transparent',
              )}
            >
        ...
        <div className="space-y-1">
          <div className={cn('h-1.5 rounded-full overflow-hidden', isDark ? 'bg-zinc-800' : 'bg-gray-200')}>
            <div
              ...
              style={{ width: `${entries.length > 0 ? (countDone / entries.length) * 100 : 0}%` }}
            />
          </div>
          <div className="flex justify-between">
            <span className={cn('text-[8px]', isDark ? 'text-zinc-500' : 'text-gray-400')}>
              {countDone}/{entries.length} completadas
            </span>
            <span className={cn('text-[8px] font-mono', isDark ? 'text-zinc-500' : 'text-gray-400')}>
      ...
      <div className="space-y-1">
        ...
        {filtered.length === 0 && (
          <div className={cn('text-center py-6 text-[10px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>
            {search ? 'Sin resultados' : 'No hay tickets aún'}
          </div>
        )}
      </div>
      ...
      {/* Add new ticket */}
      {addingNew ? (
        <div className={cn('rounded-lg border p-2.5 space-y-2', isDark ? 'border-violet-700/40 bg-zinc-900' : 'border-violet-200 bg-gray-50')}>
          <AutoResizeTextarea
            className={cn(inputClass, 'w-full text-[11px]')}
            placeholder="Descripción del ticket..."
            value={timeDesc}
            onChange={e => setTimeDesc(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleAddTime(); setAddingNew(false); } }}
            autoFocus
          />
          <div className="flex items-center gap-2">
            <div className={cn(
              ...
              <button
                onClick={() => setTimeHours(String(Math.max(0, (parseFloat(timeHours) || 0) - 0.5)))}
                className={cn('px-1.5 py-0.5 rounded-l-md transition-colors', isDark ? 'hover:bg-zinc-700 text-zinc-400' : 'hover:bg-gray-100 text-gray-500')}
              >
                <span className="text-[11px] font-bold">−</span>
              </button>
              <span className={cn('text-[10px] font-mono font-bold px-1 min-w-[28px] text-center tabular-nums', isDark ? 'text-zinc-200' : 'text-gray-700')}>
                {(parseFloat(timeHours) || 0).toFixed(1)}h
              </span>
              <button
                onClick={() => setTimeHours(String((parseFloat(timeHours) || 0) + 0.5))}
                className={cn('px-1.5 py-0.5 rounded-r-md transition-colors', isDark ? 'hover:bg-zinc-700 text-zinc-400' : 'hover:bg-gray-100 text-gray-500')}
              >
                <span className="text-[11px] font-bold">+</span>
              </button>
            </div>
            <div className="flex-1" />
            <button onClick={() => { setAddingNew(false); setTimeDesc(''); setTimeHours(''); }} className={cn('p-1 rounded text-[10px]', isDark ? 'text-zinc-400' : 'text-gray-500')}>
              <X className="size-3" />
            </button>
            <button
              onClick={() => { handleAddTime(); setAddingNew(false); }}
              disabled={!timeDesc.trim()}
              className={cn(
                'px-3 py-1 rounded-md text-[10px] font-medium transition-colors',
                timeDesc.trim() ? 'bg-violet-600 text-white hover:bg-violet-500' : isDark ? 'bg-zinc-800 text-zinc-600' : 'bg-gray-100 text-gray-400',
              )}
            >
              Añadir
            </button>
        ...
        <button
          onClick={() => { setAddingNew(true); setTimeHours('1'); }}
          className={cn(
            'w-full flex items-center justify-center gap-1.5 rounded-lg border border-dashed py-2 text-[10px] font-medium transition-all',
            isDark ? 'border-zinc-700 text-zinc-500 hover:border-violet-600 hover:text-violet-400 hover:bg-violet-950/20' : 'border-gray-300 text-gray-400 hover:border-violet-400 hover:text-violet-600 hover:bg-violet-50',
          )}
        >
...
function AiSearchPanel({ isDark, t, onTicketAccepted }: { isDark: boolean; t: (k: string) => string; onTicketAccepted: () => void }) {
  const [session, setSession] = useState<AiSearchSession | null>(null);
  const [searchTickets, setSearchTickets] = useState<AiSearchTicket[]>([]);
  const [durationMinutes, setDurationMinutes] = useState(5);
  const [category, setCategory] = useState<string>('both');
  ...
  const connectSSE = useCallback(() => {
    if (eventSourceCleanup.current) eventSourceCleanup.current();
    const cleanup = agentApi.kanbanAiSearchStream((event) => {
      if (event.type === 'init') {
        const s = event.session as AiSearchSession | null;
        setSession(s);
        setSearchTickets((event.tickets || []) as AiSearchTicket[]);
        if (s) {
          setElapsedMinutes(s.elapsedMinutes || 0);
          setCategory(s.category);
          setDurationMinutes(s.durationMinutes);
        }
      } else if (event.type === 'new_ticket') {
        const ticket = event.ticket as AiSearchTicket;
        setSearchTickets((prev) => {
          if (prev.some((t) => t.id === ticket.id)) return prev;
          return [...prev, ticket];
        });
      } else if (event.type === 'ticket_accepted') {
        setSearchTickets((prev) => prev.map((t) => t.id === event.ticketId ? { ...t, reviewStatus: 'accepted' } : t));
        onTicketAccepted();
      } else if (event.type === 'ticket_rejected') {
        setSearchTickets((prev) => prev.filter((t) => t.id !== event.ticketId));
      } else if (event.type === 'session_completed' || event.type === 'session_cancelled') {
        setSession((prev) => prev ? { ...prev, status: event.type === 'session_completed' ? 'completed' : 'cancelled' } : null);
      }
    });
    eventSourceCleanup.current = cleanup;
  }, [onTicketAccepted]);

  useEffect(() => {
    connectSSE();
    return () => { if (eventSourceCleanup.current) eventSourceCleanup.current(); };
  }, [connectSSE]);

  useEffect(() => {
    if (session?.status === 'running') {
      progressTimerRef.current = setInterval(() => {
        setElapsedMinutes((Date.now() - new Date(session.startedAt).getTime()) / 60000);
      }, 1000);
    }
  ...
  const handleStart = async () => {
    setStarting(true);
    try {
      const res = await agentApi.kanbanAiSearchStart(durationMinutes, category);
      setSession(res.session as AiSearchSession);
      setSearchTickets([]);
      setElapsedMinutes(0);
    } catch { /* ignore */ }
    setStarting(false);
  ...
  const handleStop = async () => {
    try {
      await agentApi.kanbanAiSearchStop();
  ...
  const handleAccept = async (ticketId: string) => {
    ...
    try {
      await agentApi.kanbanAiSearchAcceptTicket(ticketId);
    } catch { /* ignore */ }
  ...
  const handleReject = async (ticketId: string) => {
    setActionLoading(ticketId);
    try {
      await agentApi.kanbanAiSearchRejectTicket(ticketId);
    } catch { /* ignore */ }
    setActionLoading(null);
  };

  const progress = session ? Math.min(100, (elapsedMinutes / session.durationMinutes) * 100) : 0;
  const remaining = session ? Math.max(0, session.durationMinutes - elapsedMinutes) : 0;
  const pendingTickets = searchTickets.filter((t) => t.reviewStatus === 'pending');
  const acceptedTickets = searchTickets.filter((t) => t.reviewStatus === 'accepted');
  const isRunning = session?.status === 'running';
  const isCompleted = session?.status === 'completed';
  const isCancelled = session?.status === 'cancelled';

  const categoryOptions = [
    { key: 'frontend', label: t('kanbanAiSearchFrontend'), color: 'text-blue-400', bg: isDark ? 'bg-blue-600/20 border-blue-700/40' : 'bg-blue-50 border-blue-200' },
    { key: 'backend', label: t('kanbanAiSearchBackend'), color: 'text-emerald-400', bg: isDark ? 'bg-emerald-600/20 border-emerald-700/40' : 'bg-emerald-50 border-emerald-200' },
    { key: 'both', label: t('kanbanAiSearchBoth'), color: 'text-violet-400', bg: isDark ? 'bg-violet-600/20 border-violet-700/40' : 'bg-violet-50 border-violet-200' },
    { key: 'security', label: t('kanbanAiSearchSecurity'), color: 'text-red-400', bg: isDark ? 'bg-red-600/20 border-red-700/40' : 'bg-red-50 border-red-200' },
  ];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* Setup / Status header */}
      <div className={cn('px-3 py-3 border-b space-y-3', isDark ? 'border-zinc-800 bg-zinc-900/60' : 'border-gray-200 bg-gray-50/60')}>
        {!isRunning && !isCompleted && !isCancelled ? (
          <>
            {/* Duration slider */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className={cn('text-[10px] font-medium', isDark ? 'text-zinc-400' : 'text-gray-500')}>
                  {t('kanbanAiSearchMinutes')}
                </span>
                <span className={cn('text-[11px] font-bold tabular-nums', isDark ? 'text-cyan-300' : 'text-cyan-600')}>
                  {durationMinutes} min
                </span>
              </div>
              <input
              ...
              />
              <div className={cn('flex justify-between text-[9px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>
                ...
                {categoryOptions.map((opt) => (
                  <button
                    ...
                    className={cn(
                      ...
                      category === opt.key
                        ? cn(opt.bg, opt.color, 'ring-1', isDark ? 'ring-cyan-500/40' : 'ring-cyan-500/30')
                        : isDark ? 'border-zinc-700 text-zinc-500 hover:text-zinc-300 hover:border-zinc-600' : 'border-gray-200 text-gray-400 hover:text-gray-600 hover:border-gray-300',
                    )}
                  >
                    {opt.label}
                  </button>
            ...
            <button
              ...
              className={cn(
                ...
                starting
                  ...
                  : isDark ? 'bg-cyan-600/20 text-cyan-300 hover:bg-cyan-600/30 border border-cyan-700/40' : 'bg-cyan-50 text-cyan-600 hover:bg-cyan-100 border border-cyan-200',
              )}
            >
              {starting ? <Loader2 className="size-3.5 animate-spin" /> : <Play className="size-3.5" />}
              {t('kanbanAiSearchStart')}
            </button>
          </>
        ) : (
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {isRunning && <Loader2 className={cn('size-3.5 animate-spin', isDark ? 'text-cyan-400' : 'text-cyan-500')} />}
                  {isCompleted && <Check className="size-3.5 text-emerald-400" />}
                  {isCancelled && <X className="size-3.5 text-amber-400" />}
                  <span className={cn('text-[11px] font-semibold', isDark ? 'text-zinc-200' : 'text-gray-700')}>
                    {isRunning ? t('kanbanAiSearchRunning') : isCompleted ? t('kanbanAiSearchCompleted') : t('kanbanAiSearchCancelledInfo')}
                  </span>
                </div>
                <span className={cn('text-[10px] font-medium px-1.5 py-0.5 rounded', isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-100 text-gray-500')}>
                  {categoryOptions.find((o) => o.key === (session?.category || category))?.label}
                </span>
              </div>
              ...
              <div className="space-y-1">
                <div className={cn('w-full h-2 rounded-full overflow-hidden', isDark ? 'bg-zinc-800' : 'bg-gray-200')}>
                  <div
                    className={cn(
                      ...
                      isCompleted ? 'bg-emerald-500' : isCancelled ? 'bg-amber-500' : 'bg-cyan-500',
                    )}
                ...
                <div className="flex justify-between">
                  <span className={cn('text-[9px]', isDark ? 'text-zinc-500' : 'text-gray-400')}>
                    {t('kanbanAiSearchElapsed')}: {Math.floor(elapsedMinutes)}m {Math.floor((elapsedMinutes % 1) * 60)}s
                  </span>
                  {isRunning && (
                    <span className={cn('text-[9px]', isDark ? 'text-zinc-500' : 'text-gray-400')}>
                      {t('kanbanAiSearchRemaining')}: {Math.floor(remaining)}m {Math.floor((remaining % 1) * 60)}s
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <span className={cn('text-[10px]', isDark ? 'text-zinc-400' : 'text-gray-500')}>
                  {searchTickets.length} {t('kanbanAiSearchGenerated')} &middot; {acceptedTickets.length} {t('kanbanAiSearchAccept').toLowerCase()}
                </span>
                {isRunning && (
                  <button
                    onClick={handleStop}
                    className={cn(
                      ...
                      isDark ? 'bg-red-600/20 text-red-400 hover:bg-red-600/30' : 'bg-red-50 text-red-500 hover:bg-red-100',
                    )}
                  >
                    <X className="size-3" />
                    {t('kanbanAiSearchStop')}
                  </button>
                )}
                {(isCompleted || isCancelled) && (
                  <button
                    onClick={() => { setSession(null); setSearchTickets([]); }}
                    className={cn(
                      'flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium transition-colors',
                      isDark ? 'bg-cyan-600/20 text-cyan-300 hover:bg-cyan-600/30' : 'bg-cyan-50 text-cyan-600 hover:bg-cyan-100',
                    )}
                  >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1.5">
        {pendingTickets.length === 0 && acceptedTickets.length === 0 && (
          <div className={cn('flex flex-col items-center justify-center py-12', isDark ? 'text-zinc-600' : 'text-gray-400')}>
            <Sparkles className="size-8 mb-2 opacity-30" />
            <p className="text-[11px]">{isRunning ? t('kanbanAiSearchRunning') : t('kanbanAiSearchEmpty')}</p>
          </div>
        )}
        ...
        {pendingTickets.map((ticket) => (
          <div
            ...
            className={cn(
              ...
              isDark ? 'border-zinc-700/60 bg-zinc-900/80 hover:border-zinc-600' : 'border-gray-200 bg-white hover:border-gray-300',
            )}
          >
            {/* Ticket header */}
            <button
              onClick={() => setExpandedTicket(expandedTicket === ticket.id ? null : ticket.id)}
              className={cn('w-full flex items-center gap-2 px-3 py-2 text-left')}
            >
              <div className={cn('size-2 rounded-full shrink-0', priorityColor(ticket.priority))} />
              <span className={cn('flex-1 text-[11px] font-medium truncate', isDark ? 'text-zinc-200' : 'text-gray-800')}>
                {ticket.title}
              </span>
              <div className="flex items-center gap-1 shrink-0">
                {ticket.tags?.slice(0, 2).map((tag) => (
                  <span key={tag} className={cn('text-[8px] px-1 py-0.5 rounded', isDark ? 'bg-zinc-800 text-zinc-500' : 'bg-gray-100 text-gray-500')}>
                    {tag}
                  </span>
                ))}
                {expandedTicket === ticket.id ? <ChevronDown className="size-3 text-zinc-500" /> : <ChevronRight className="size-3 text-zinc-500" />}
              </div>
            </button>

            {/* Expanded content */}
            {expandedTicket === ticket.id && (
              <div className={cn('px-3 pb-3 space-y-2', isDark ? 'border-t border-zinc-800' : 'border-t border-gray-100')}>
                <div className={cn('pt-2 text-[10px] whitespace-pre-wrap', isDark ? 'text-zinc-400' : 'text-gray-600')}>
                  {ticket.description}
                </div>
                <div className="flex flex-wrap gap-1">
                  {ticket.tags?.map((tag) => (
                    <span key={tag} className={cn('text-[9px] px-1.5 py-0.5 rounded-full', isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-100 text-gray-500')}>
                      {tag}
                    </span>
                  ))}
                  {ticket.group && (
                    <span className={cn('text-[9px] px-1.5 py-0.5 rounded-full', isDark ? 'bg-violet-900/40 text-violet-400' : 'bg-violet-50 text-violet-600')}>
                      {ticket.group}
                    </span>
                  )}
                </div>
            ...
            {/* Action buttons */}
            <div className={cn('flex items-center gap-1 px-2 py-1.5', isDark ? 'bg-zinc-950/50' : 'bg-gray-50')}>
              <button
              ...
              >
                {actionLoading === ticket.id ? <Loader2 className="size-3 animate-spin" /> : <Check className="size-3" />}
                {t('kanbanAiSearchAccept')}
              </button>
              <button
                ...
                className={cn(
                  'flex-1 flex items-center justify-center gap-1 py-1 rounded text-[10px] font-medium transition-colors',
                  isDark ? 'bg-red-600/15 text-red-400 hover:bg-red-600/25' : 'bg-red-50 text-red-500 hover:bg-red-100',
                )}
              >
                <Trash2 className="size-3" />
                {t('kanbanAiSearchReject')}
              </button>
            </div>
          </div>
        ))}

        {/* Accepted tickets (collapsed) */}
        {acceptedTickets.length > 0 && (
          <div className={cn('mt-3 rounded-lg border p-2', isDark ? 'border-emerald-800/40 bg-emerald-950/20' : 'border-emerald-200 bg-emerald-50/50')}>
            <p className={cn('text-[10px] font-medium mb-1', isDark ? 'text-emerald-400' : 'text-emerald-600')}>
              {t('kanbanAiSearchAccepted')} ({acceptedTickets.length})
            </p>
            {acceptedTickets.map((ticket) => (
              <div key={ticket.id} className="flex items-center gap-1.5 py-0.5">
                <Check className="size-3 text-emerald-500 shrink-0" />
                <span className={cn('text-[10px] truncate', isDark ? 'text-emerald-300/70' : 'text-emerald-700/70')}>
                  {ticket.title}
                </span>
              </div>
            ))}
...
export function KanbanBoard({ onCreateAgent, onGoToAgent, initialTicketId, onInitialTicketHandled }: KanbanBoardProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className={cn(
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium transition-colors',
            view === 'ai_search'
              ? isDark ? 'bg-cyan-600/20 text-cyan-300' : 'bg-cyan-100 text-cyan-600'
              : isDark ? 'text-zinc-500 hover:text-zinc-300' : 'text-gray-400 hover:text-gray-700',
          )}
        >
          <Sparkles className="size-3" />
          {t('kanbanAiSearchTitle')}
        </button>
      ...
      {/* Toolbar — search, filter, sort, create (hidden during AI search) */}
      {view !== 'ai_search' && <div className={cn(
      ...
      {view === 'board' ? (
        ...
        <AiSearchPanel isDark={isDark} t={t} onTicketAccepted={loadData} />
      )}
...
function TableView({ tickets, isDark, t, onSelect, onDelete }: { tickets: KanbanTicket[]; isDark: boolean; t: (key: string) => string; onSelect: (t: KanbanTicket) => void; onDelete: (id: string) => void }) {
  const hdrCls = cn('text-[9px] font-semibold uppercase tracking-wider px-3 py-2.5 text-left', isDark ? 'text-zinc-500' : 'text-gray-400');
  const cellCls = cn('px-3 py-2.5 text-[10px]', isDark ? 'text-zinc-300' : 'text-gray-700');

  return (
    <div className="flex-1 overflow-auto min-h-0">
      <table className="w-full min-w-[650px]">
        <thead className={cn('sticky top-0 z-10', isDark ? 'bg-zinc-900/95 backdrop-blur-sm' : 'bg-gray-50/95 backdrop-blur-sm')}>
          <tr className={cn('border-b', isDark ? 'border-zinc-800' : 'border-gray-200')}>
            <th className={hdrCls}>{t('kanbanTitle')}</th>
            <th className={cn(hdrCls, 'w-24')}>{t('kanbanStatus')}</th>
            <th className={cn(hdrCls, 'w-16')}>{t('kanbanPriority')}</th>
            <th className={cn(hdrCls, 'w-28')}>{t('kanbanAssignee')}</th>
            <th className={cn(hdrCls, 'w-20')}>Fecha</th>
            <th className={cn(hdrCls, 'w-20')}>Tags</th>
        ...
        <tbody>
          {tickets.map((ticket) => {
            ...
            const isOverdue = ticket.endDate && new Date(ticket.endDate) < new Date() && ticket.status !== 'done';
            const st = STATUSES.find(s => s.key === ticket.status);
            return (
              <tr key={ticket.id}
                className={cn('border-b cursor-pointer transition-colors group border-l-[3px]',
                  isDark ? 'border-b-zinc-800/60 hover:bg-zinc-800/40' : 'border-b-gray-100 hover:bg-gray-50',
                  prio?.key === 'critical' ? 'border-l-red-500' : prio?.key === 'high' ? 'border-l-orange-500' : prio?.key === 'medium' ? 'border-l-yellow-500' : 'border-l-green-500',
                )}
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        {ticket.ticketNumber && (
                          <span className={cn('text-[8px] font-mono shrink-0', isDark ? 'text-zinc-600' : 'text-gray-400')}>AYT-{ticket.ticketNumber}</span>
                        )}
                        {ticket.group && (
                          <span className={cn('text-[7px] font-medium px-1 py-0.5 rounded shrink-0', isDark ? 'bg-sky-900/30 text-sky-400' : 'bg-sky-50 text-sky-600')}>
                            {ticket.group}
                          </span>
                        )}
                      </div>
                      <span className="font-semibold truncate block text-[11px]">{ticket.title}</span>
                      {ticket.description && (
                        <span className={cn('text-[9px] truncate block mt-0.5', isDark ? 'text-zinc-500' : 'text-gray-400')}>
                          {ticket.description.replace(/[#*_`>\-\[\]x]/g, '').replace(/\n/g, ' ').trim().slice(0, 80)}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      {(ticket.comments?.length || 0) > 0 && (
                        <span className={cn('flex items-center gap-0.5 text-[8px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>
                          <MessageSquare className="size-2.5" />{ticket.comments.length}
                        </span>
                      )}
                      {(ticket.attachments?.length || 0) > 0 && (
                        <span className={cn('flex items-center gap-0.5 text-[8px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>
                          <Paperclip className="size-2.5" />{ticket.attachments.length}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <td className={cellCls}>
                  <span className={cn('inline-flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-md', isDark ? st?.bgDark : st?.bgLight, st?.color)}>
                    <span className={cn('size-1.5 rounded-full', STATUS_DOT[ticket.status])} />
                    {t(`kanbanStatus_${ticket.status}`)}
                  </span>
                </td>
                <td className={cellCls}>
                  <span className={cn('inline-flex items-center gap-1 text-[9px] font-bold', prio?.color)}>
                    <div className={cn('size-2 rounded-full', prio?.dot || 'bg-gray-400')} />
                    {prio?.key === 'critical' ? 'Crit' : prio?.key === 'high' ? 'High' : prio?.key === 'medium' ? 'Med' : 'Low'}
                  </span>
                </td>
                <td className={cellCls}>
                  {ticket.assignee ? (
                    <span className="flex items-center gap-1.5">
                      <span className={cn('size-5 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0 ring-1',
                        isDark ? 'bg-violet-600/30 text-violet-300 ring-violet-600/20' : 'bg-violet-100 text-violet-600 ring-violet-200')}>
                        {ticket.assignee.charAt(0).toUpperCase()}
                      </span>
                      <span className="truncate text-[9px] font-medium">{ticket.assignee}</span>
                    </span>
                  ) : <span className={cn('text-[9px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>—</span>}
                </td>
                <td className={cellCls}>
                  {ticket.endDate ? (
                    <span className={cn('flex items-center gap-0.5 text-[9px] font-medium',
                      isOverdue
                        ? isDark ? 'text-red-400' : 'text-red-500'
                        : isDark ? 'text-zinc-400' : 'text-gray-500')}>
                      <Calendar className="size-2.5" />
                      {new Date(ticket.endDate).toLocaleDateString('es', { day: 'numeric', month: 'short' })}
                    </span>
                  ) : <span className={cn('text-[9px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>—</span>}
                </td>
                <td className={cellCls}>
                  <div className="flex gap-0.5 flex-wrap">
                    {ticket.tags?.slice(0, 2).map((tag) => (
                      <span key={tag} className={cn('text-[7px] px-1 py-0.5 rounded font-medium', isDark ? 'bg-amber-900/20 text-amber-400/80' : 'bg-amber-50 text-amber-600')}>
                        {tag}
                      </span>
                    ))}
                    {(ticket.tags?.length || 0) > 2 && (
                      <span className={cn('text-[7px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>+{ticket.tags!.length - 2}</span>
                    )}
                  </div>
                </td>
                <td className={cellCls}>
                  <button
                    ...
                    className={cn('size-5 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity', isDark ? 'text-red-400 hover:bg-red-900/30' : 'text-red-500 hover:bg-red-50')}
                  >
  ...
  return (
    <div className="flex-1 overflow-y-auto min-h-0 p-2 space-y-3">
      {STATUSES.map((col) => {
        ...
        return (
          <div key={col.key}>
            <div className={cn('flex items-center gap-2 px-2 py-2 rounded-lg mb-1', isDark ? 'bg-zinc-900/60' : 'bg-gray-50')}>
              <div className={cn('size-2.5 rounded-full', STATUS_DOT[col.key])} />
              <span className={cn('text-[10px] font-bold uppercase tracking-wider', col.color)}>{t(`kanbanStatus_${col.key}`)}</span>
              <span className={cn('text-[9px] px-1.5 py-0.5 rounded-full font-semibold', isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-200 text-gray-500')}>{items.length}</span>
            </div>
              ...
              {items.map((ticket) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => onSelect(ticket)}
                    className={cn(
                      'flex items-center gap-2 px-2.5 py-2 rounded-lg cursor-pointer group transition-all border-l-[3px]',
                      isDark ? 'hover:bg-zinc-800/50' : 'hover:bg-gray-50',
                      prio?.key === 'critical' ? 'border-l-red-500' : prio?.key === 'high' ? 'border-l-orange-500' : prio?.key === 'medium' ? 'border-l-yellow-500' : 'border-l-green-500',
                    )}
                  >
                    {ticket.ticketNumber && (
                      <span className={cn('text-[8px] font-mono shrink-0 w-10', isDark ? 'text-zinc-600' : 'text-gray-400')}>AYT-{ticket.ticketNumber}</span>
                    )}
                    <div className="flex-1 min-w-0">
                      <span className={cn('text-[11px] font-semibold truncate block', isDark ? 'text-zinc-200' : 'text-gray-700')}>{ticket.title}</span>
                      {ticket.description && (
                        <span className={cn('text-[9px] truncate block', isDark ? 'text-zinc-600' : 'text-gray-400')}>
                          {ticket.description.replace(/[#*_`>\-\[\]x]/g, '').replace(/\n/g, ' ').trim().slice(0, 60)}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      {ticket.tags?.slice(0, 1).map((tag) => (
                        <span key={tag} className={cn('text-[7px] font-medium px-1 py-0.5 rounded shrink-0', isDark ? 'bg-amber-900/20 text-amber-400/80' : 'bg-amber-50 text-amber-600')}>
                          #{tag}
                        </span>
                      ))}
                      {isOverdue && (
                        <span className={cn('text-[7px] font-bold px-1 py-0.5 rounded shrink-0', isDark ? 'bg-red-900/30 text-red-400' : 'bg-red-50 text-red-500')}>
                          <Calendar className="size-2.5 inline" />
                        </span>
                      )}
                      {ticket.assignee && (
                        <span className={cn('size-5 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0 ring-1',
                          isDark ? 'bg-violet-600/20 text-violet-300 ring-violet-600/20' : 'bg-violet-50 text-violet-600 ring-violet-200')}>
                          {ticket.assignee.charAt(0).toUpperCase()}
                        </span>
                      )}
                      <div className="flex items-center gap-1">
                        {(ticket.comments?.length || 0) > 0 && (
                          <span className={cn('flex items-center gap-0.5 text-[8px] shrink-0', isDark ? 'text-zinc-600' : 'text-gray-400')}>
                            <MessageSquare className="size-2.5" />{ticket.comments.length}
                          </span>
                        )}
                        {(ticket.attachments?.length || 0) > 0 && (
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); onDelete(ticket.id); }}
                        className={cn('size-4 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shrink-0', isDark ? 'text-red-400' : 'text-red-500')}
                      >
                        <Trash2 className="size-2.5" />
                      </button>
                    </div>
            ...
            {ganttTickets.map((tk) => {
              ...
              return (
                ...
                >
                  ...
                  {tk.ticketNumber && (
                    <span className={cn('text-[8px] font-mono shrink-0', isDark ? 'text-zinc-600' : 'text-gray-400')}>AYT-{tk.ticketNumber}</span>
                  )}
                  ...
                  <div key={tk.id} className={cn('border-b', isDark ? 'border-zinc-800/20' : 'border-gray-50')} style={{ height: rowH }}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 h-full px-2 overflow-hidden">
                        {tk.ticketNumber && <span className="text-[7px] font-mono text-white/60 shrink-0">AYT-{tk.ticketNumber}</span>}
...
function TicketCard({
  ...
  const prio = PRIORITIES.find((p) => p.key === ticket.priority);
  const statusCfg = STATUSES.find((s) => s.key === ticket.status);
  ...
  const formatDate = (d?: string) => {
    if (!d) return null;
    try { return new Date(d).toLocaleDateString('es', { day: 'numeric', month: 'short' }); } catch { return null; }
  };

  const isOverdue = ticket.endDate && new Date(ticket.endDate) < new Date() && ticket.status !== 'done';
  const checklistItems = ticket.description?.match(/- \[[ x]\]/g);
  const checklistDone = ticket.description?.match(/- \[x\]/g);
  const hasChecklist = checklistItems && checklistItems.length > 0;

  const prioBg = prio?.key === 'critical'
    ? isDark ? 'bg-red-950/40 border-l-red-500' : 'bg-red-50/60 border-l-red-400'
    : prio?.key === 'high'
      ? isDark ? 'bg-orange-950/20 border-l-orange-500' : 'bg-orange-50/40 border-l-orange-400'
      : '';
  ...
  return (
    <div
      ...
      onClick={onSelect}
      className={cn(
        'group rounded-xl border cursor-pointer transition-all relative overflow-hidden border-l-[3px]',
        dragging ? 'opacity-40 scale-95' : 'hover:shadow-lg hover:-translate-y-0.5',
        agentWorking
          ? isDark ? 'bg-violet-950/30 border-violet-600/50 border-l-violet-500 shadow-violet-900/20 shadow-md' : 'bg-violet-50/60 border-violet-300 border-l-violet-500 shadow-violet-200/40 shadow-md'
          : doneByAi
            ? isDark ? 'bg-emerald-950/20 border-emerald-800/40 border-l-emerald-500 hover:border-emerald-700/60' : 'bg-emerald-50/50 border-emerald-200 border-l-emerald-400 hover:border-emerald-300'
            : prio?.key === 'critical' || prio?.key === 'high'
              ? cn(isDark ? 'border-zinc-700/50 hover:border-zinc-600' : 'border-gray-200 hover:border-gray-300', prioBg)
              : isDark ? 'bg-zinc-900/80 border-zinc-700/50 border-l-zinc-600 hover:border-zinc-600' : 'bg-white border-gray-200 border-l-gray-300 hover:border-gray-300',
      )}
    >
      {/* Agent working: animated top bar */}
      {agentWorking && (
        <div className="absolute top-0 left-0 right-0 h-1 overflow-hidden z-10">
          <div className={cn('absolute inset-0', isDark ? 'bg-violet-900/30' : 'bg-violet-100')} />
          <div className={cn('h-full w-2/5 rounded-full', isDark ? 'bg-violet-500' : 'bg-violet-400')}
            style={{ animation: 'shimmer 1.5s ease-in-out infinite' }} />
          <style>{`@keyframes shimmer { 0% { transform: translateX(-100%); } 100% { transform: translateX(400%); } }`}</style>
        </div>
      )}

      {/* Done by AI: success top bar */}
      {doneByAi && !agentWorking && (
        <div className={cn('absolute top-0 left-0 right-0 h-1 z-10', isDark ? 'bg-emerald-500/60' : 'bg-emerald-400')} />
      )}

      <div className="p-3">
        {/* Agent running: prominent banner */}
        {agentWorking && (
          <button
            onClick={(e) => { e.stopPropagation(); if (agentId && onGoToAgent) onGoToAgent(agentId); }}
            className={cn(
              'w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg mb-2 transition-all',
              isDark ? 'bg-violet-600/20 border border-violet-500/30 hover:bg-violet-600/30' : 'bg-violet-100 border border-violet-200 hover:bg-violet-200',
            )}
          >
            <div className="relative flex items-center justify-center shrink-0">
              <div className={cn('absolute size-5 rounded-full animate-ping opacity-20', isDark ? 'bg-violet-500' : 'bg-violet-400')} />
              <Bot className={cn('relative size-3.5 animate-pulse', isDark ? 'text-violet-400' : 'text-violet-600')} />
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className={cn('text-[9px] font-bold', isDark ? 'text-violet-300' : 'text-violet-700')}>Agente trabajando...</p>
            </div>
            {agentId && onGoToAgent && (
              <span className={cn('text-[8px] font-semibold', isDark ? 'text-violet-400' : 'text-violet-500')}>Ver →</span>
            )}
          </button>
        )}

        {/* Done by AI: success banner */}
        {doneByAi && !agentWorking && (
          <div className={cn(
            'flex items-center gap-2 px-2.5 py-1.5 rounded-lg mb-2',
            isDark ? 'bg-emerald-900/25 border border-emerald-700/30' : 'bg-emerald-50 border border-emerald-200',
          )}>
            <div className={cn('size-5 rounded-full flex items-center justify-center shrink-0', isDark ? 'bg-emerald-600/30' : 'bg-emerald-200')}>
              <Check className={cn('size-3', isDark ? 'text-emerald-400' : 'text-emerald-600')} />
            </div>
            <p className={cn('text-[9px] font-bold', isDark ? 'text-emerald-400' : 'text-emerald-700')}>Completado por IA</p>
            <Bot className={cn('size-3 ml-auto shrink-0', isDark ? 'text-emerald-500' : 'text-emerald-500')} />
          </div>
        )}

        {/* Top row: ticket# + badges + menu */}
        <div className="flex items-center gap-1.5 mb-2">
          <GripVertical className={cn('size-3 shrink-0 opacity-0 group-hover:opacity-40 cursor-grab -ml-1', isDark ? 'text-zinc-600' : 'text-gray-400')} />
          {ticket.ticketNumber && (
            <span className={cn('text-[9px] font-mono font-medium', isDark ? 'text-zinc-500' : 'text-gray-400')}>
              AYT-{ticket.ticketNumber}
            </span>
          )}
          <div className="flex-1" />
          ...
          <div className="relative">
            ...
            {showMenu && (
              <>
                ...
                <div className={cn('absolute right-0 top-6 z-50 rounded-lg shadow-xl py-1 min-w-[100px] border', isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-gray-200')}>
                  <button
                    onClick={(e) => { e.stopPropagation(); onDelete(); setShowMenu(false); }}
                    className="flex items-center gap-2 w-full px-3 py-1.5 text-[10px] text-red-400 hover:bg-red-500/10"
                  >
        ...
        {/* Title */}
        <p className={cn('text-[13px] font-semibold leading-snug mb-1', isDark ? 'text-zinc-100' : 'text-gray-900')}>
          {ticket.title}
        </p>

        {/* Description preview */}
        {ticket.description && (
          <p className={cn('text-[10px] leading-relaxed line-clamp-2 mb-2', isDark ? 'text-zinc-500' : 'text-gray-500')}>
            {ticket.description.replace(/[#*_`>\-\[\]x]/g, '').replace(/\n/g, ' ').trim().slice(0, 120)}
          </p>
        )}

        {/* Priority + Status + Group badges row */}
        <div className="flex items-center gap-1 flex-wrap mb-2">
          <span className={cn('inline-flex items-center gap-1 text-[8px] font-bold px-1.5 py-0.5 rounded-md shrink-0', prio?.color || '', isDark ? 'bg-zinc-800' : 'bg-gray-100')}>
            <div className={cn('size-1.5 rounded-full', prio?.dot || 'bg-gray-400')} />
            {prio?.key === 'critical' ? 'CRIT' : prio?.key === 'high' ? 'HIGH' : prio?.key === 'medium' ? 'MED' : 'LOW'}
          </span>
          {statusCfg && (
            <span className={cn('inline-flex items-center gap-1 text-[8px] font-semibold px-1.5 py-0.5 rounded-md', isDark ? statusCfg.bgDark : statusCfg.bgLight, statusCfg.color)}>
              <div className={cn('size-1.5 rounded-full', STATUS_DOT[ticket.status])} />
              {ticket.status === 'in_progress' ? 'Prog' : ticket.status.charAt(0).toUpperCase() + ticket.status.slice(1)}
            </span>
          )}
          {ticket.group && (
            <span className={cn('text-[8px] font-medium px-1.5 py-0.5 rounded-md', isDark ? 'bg-sky-900/30 text-sky-400' : 'bg-sky-50 text-sky-600')}>
              {ticket.group}
            </span>
          )}
          {ticket.subgroup && (
            <span className={cn('text-[8px] font-medium px-1.5 py-0.5 rounded-md', isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-100 text-gray-500')}>
              {ticket.subgroup}
            </span>
          )}
        </div>
        ...
        {ticket.tags?.length > 0 && (
          <div className="flex items-center gap-1 flex-wrap mb-2">
            {ticket.tags.slice(0, 4).map((tag) => (
              <span key={tag} className={cn('text-[8px] font-medium px-1.5 py-0.5 rounded-md', isDark ? 'bg-amber-900/20 text-amber-400/80' : 'bg-amber-50 text-amber-600')}>
                #{tag}
              </span>
            ))}
            {ticket.tags.length > 4 && (
              <span className={cn('text-[8px]', isDark ? 'text-zinc-500' : 'text-gray-400')}>+{ticket.tags.length - 4}</span>
            )}
          </div>
        )}

        {/* Progress indicators: AI + checklist + study badge */}
        <div className="flex items-center gap-2 mb-2">
          <div className="flex items-center gap-1.5 flex-1">
            ...
            <Sparkles className={cn('size-2.5', aiColors.text, aiScore >= 75 && 'animate-pulse')} />
            <span className={cn('text-[8px] font-mono tabular-nums', aiColors.text)}>{aiScore}%</span>
          </div>
          {ticket.aiStudy && !agentWorking && !doneByAi && (
            <span className={cn('flex items-center gap-0.5 text-[7px] font-bold px-1.5 py-0.5 rounded-md shrink-0', isDark ? 'bg-amber-900/30 text-amber-400 border border-amber-700/30' : 'bg-amber-50 text-amber-600 border border-amber-200')}>
              <Eye className="size-2.5" /> Estudiado
            </span>
          )}
          {hasChecklist && (
            <span className={cn('flex items-center gap-0.5 text-[8px] font-medium px-1.5 py-0.5 rounded-md shrink-0',
              isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-100 text-gray-500')}>
              <ListChecks className="size-2.5" />
              {checklistDone?.length || 0}/{checklistItems.length}
            </span>
          )}
        </div>

        {/* Footer: assignee, date, meta */}
        <div className={cn('flex items-center gap-2 pt-2 border-t', isDark ? 'border-zinc-800/60' : 'border-gray-100')}>
          {ticket.assignee ? (
            <div className="flex items-center gap-1.5 min-w-0">
              <div className={cn('size-5 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0 ring-2',
                isDark ? 'bg-violet-600/30 text-violet-300 ring-violet-600/20' : 'bg-violet-100 text-violet-600 ring-violet-200/50')}>
                {ticket.assignee.charAt(0).toUpperCase()}
              </div>
              <span className={cn('text-[9px] font-medium truncate max-w-[70px]', isDark ? 'text-zinc-300' : 'text-gray-600')}>{ticket.assignee}</span>
            </div>
          ) : (
            <div className={cn('flex items-center gap-1 text-[9px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>
          ...
          <div className="flex items-center gap-1.5">
            {formatDate(ticket.endDate) && (
              <span className={cn('flex items-center gap-0.5 text-[8px] font-medium px-1 py-0.5 rounded',
                isOverdue
            ...
            {(ticket.comments?.length || 0) > 0 && (
              <span className={cn('flex items-center gap-0.5 text-[8px] font-medium', isDark ? 'text-zinc-500' : 'text-gray-400')}>
...
const AI_STEPS_CREATE = [
  { key: 'analyzing', icon: '🔍', duration: 1200 },
  { key: 'generating', icon: '✍️', duration: 1800 },
  { key: 'prioritizing', icon: '🏷️', duration: 1400 },
  { key: 'finalizing', icon: '✅', duration: 800 },
];

const AI_STEPS_LABELS: Record<string, Record<string, string>> = {
  analyzing:   { es: 'Analizando el prompt...', en: 'Analyzing prompt...' },
  generating:  { es: 'Generando título y descripción...', en: 'Generating title & description...' },
  prioritizing:{ es: 'Asignando prioridad y etiquetas...', en: 'Assigning priority & tags...' },
  finalizing:  { es: 'Finalizando ticket...', en: 'Finalizing ticket...' },
  done:        { es: 'Ticket generado', en: 'Ticket generated' },
};

const AI_STUDY_STEPS = [
  { key: 'reading', icon: '📖', duration: 1000 },
  { key: 'analyzing_tech', icon: '🔬', duration: 1600 },
  { key: 'planning', icon: '📋', duration: 1800 },
  { key: 'estimating', icon: '⏱️', duration: 1200 },
  { key: 'writing', icon: '✍️', duration: 1000 },
];

const AI_STUDY_LABELS: Record<string, Record<string, string>> = {
  reading:        { es: 'Leyendo contexto del ticket...', en: 'Reading ticket context...' },
  analyzing_tech: { es: 'Analizando requisitos técnicos...', en: 'Analyzing technical requirements...' },
  planning:       { es: 'Planificando pasos de implementación...', en: 'Planning implementation steps...' },
  estimating:     { es: 'Estimando tiempos y riesgos...', en: 'Estimating time & risks...' },
  writing:        { es: 'Redactando estudio...', en: 'Writing study...' },
  done:           { es: 'Estudio completado', en: 'Study completed' },
};

function useAiSteps(steps: { key: string; duration: number }[], active: boolean) {
...
function AiStepsIndicator({
  ...
  steps: { key: string; icon: string; duration: number }[];
  ...
  const lang = 'es';
  return (
    <div className={cn('rounded-lg border p-2.5 space-y-1.5', isDark ? 'border-zinc-700/60 bg-zinc-900/60' : 'border-violet-200 bg-violet-50/40')}>
      {steps.map((step, i) => {
      ...
      {done && (
        <div className={cn('flex items-center gap-2 px-2 py-1 rounded mt-1', isDark ? 'bg-emerald-900/20' : 'bg-emerald-50')}>
          <Check className="size-3 text-emerald-500 shrink-0" />
          <span className={cn('text-[10px] font-semibold', isDark ? 'text-emerald-400' : 'text-emerald-600')}>
            {labels['done']?.[lang] || 'Done'}
          </span>
        </div>
      )}
    </div>
...
function AiCreateDialog({
  ...
  const { currentStep, done: stepsDone, markDone } = useAiSteps(AI_STEPS_CREATE, generating);
    ...
    <div className={cn('absolute inset-0 z-[80] flex flex-col', isDark ? 'bg-zinc-950' : 'bg-white')}>
      ...
      <div className={cn('flex-1 overflow-y-auto p-3 space-y-3', isDark ? 'bg-zinc-950' : 'bg-white')}>
        ...
        {generating && currentStep >= 0 && (
          <AiStepsIndicator steps={AI_STEPS_CREATE} labels={AI_STEPS_LABELS} currentStep={currentStep} done={stepsDone} isDark={isDark} />
        )}
...
function TicketDetailPanel({
  ...
  const [activeTab, setActiveTab] = useState<'detail' | 'tickets' | 'comments' | 'history' | 'ai'>('detail');
  const [editing, setEditing] = useState(false);
  ...
  const { currentStep: studyStep, done: studyStepsDone, markDone: markStudyDone } = useAiSteps(AI_STUDY_STEPS, aiGenerating);
  const [summarizing, setSummarizing] = useState(false);
  const [summaryPreview, setSummaryPreview] = useState<string | null>(null);
  const [summaryTasks, setSummaryTasks] = useState<Array<{ title: string; description: string; priority: string; tags: string[] }>>([]);
  ...
  const handleAddTime = async () => {
    const h = parseFloat(timeHours) || 1;
    if (h < 0) return;
    const desc = timeDesc.trim();
    if (!desc) return;
    try {
  ...
  const ticketEntryCount = (ticket.timelog || []).length;
  const tabs = [
    { k: 'detail' as const, label: t('kanbanDetail'), icon: <Eye className="size-3" /> },
    { k: 'tickets' as const, label: `Tickets (${ticketEntryCount})`, icon: <ListTodo className="size-3" /> },
  ...
  const handleApplySummary = async () => {
    ...
    if (summaryTasks.length > 0) {
      for (const task of summaryTasks) {
        try {
          await agentApi.kanbanCreateTicket({
            title: task.title,
            description: task.description,
            priority: (task.priority || 'medium') as KanbanTicket['priority'],
            tags: task.tags || [],
      ...
      for (const task of summaryTasks) {
        try {
          await agentApi.kanbanAddTimeEntry(ticket.id, {
            userId: 'ai-agent',
            userName: 'AI Summarize',
            hours: 0,
            description: `[${task.priority || 'medium'}] ${task.title}${task.description ? '\n' + task.description : ''}`,
            date: today,
          });
        } catch { /* skip */ }
      }
    ...
    <div className={cn('absolute inset-0 z-[80] flex flex-col', isDark ? 'bg-zinc-950' : 'bg-white')}>
      <div className={cn('flex items-center justify-between px-3 py-2 border-b shrink-0', isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200')}>
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          {ticket.ticketNumber && (
            <span className={cn('text-[10px] font-mono shrink-0 px-1 py-0.5 rounded', isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-100 text-gray-500')}>
              AYT-{ticket.ticketNumber}
            </span>
          )}
      ...
      {/* Agent status banner */}
      {isAgentWorking && (
        <div className={cn('shrink-0 overflow-hidden', isDark ? 'bg-violet-950/50' : 'bg-violet-50')}>
          <div className="relative h-1 overflow-hidden">
            <div className={cn('absolute inset-0', isDark ? 'bg-violet-900/40' : 'bg-violet-100')} />
          ...
          <div className="px-3 py-2.5 flex items-center gap-3">
            <div className="relative flex items-center justify-center shrink-0">
              <div className={cn('absolute size-8 rounded-full animate-ping opacity-15', isDark ? 'bg-violet-500' : 'bg-violet-400')} />
              <div className={cn('relative size-8 rounded-full flex items-center justify-center', isDark ? 'bg-violet-600/40 ring-2 ring-violet-500/30' : 'bg-violet-200 ring-2 ring-violet-300/50')}>
                <Bot className={cn('size-4 animate-pulse', isDark ? 'text-violet-300' : 'text-violet-600')} />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className={cn('text-[9px]', isDark ? 'text-violet-400/60' : 'text-violet-500/80')}>
                Implementando cambios — se moverá a revisión al terminar
              </p>
            </div>
            {agentId && onGoToAgent && (
              <button
                ...
                className={cn(
                  'flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all shrink-0',
                  isDark
      ...
      {/* Done by AI banner */}
      {doneByAi && !isAgentWorking && (
        <div className={cn('shrink-0', isDark ? 'bg-emerald-950/30' : 'bg-emerald-50')}>
          <div className={cn('h-1', isDark ? 'bg-emerald-500/50' : 'bg-emerald-400/70')} />
          <div className="px-3 py-2 flex items-center gap-3">
            <div className={cn('size-7 rounded-full flex items-center justify-center shrink-0', isDark ? 'bg-emerald-600/30 ring-2 ring-emerald-500/20' : 'bg-emerald-200 ring-2 ring-emerald-300/50')}>
              <Check className={cn('size-3.5', isDark ? 'text-emerald-400' : 'text-emerald-600')} />
            </div>
            <div className="flex-1 min-w-0">
              <p className={cn('text-[11px] font-bold', isDark ? 'text-emerald-300' : 'text-emerald-700')}>
                Completado por IA
              </p>
              <p className={cn('text-[9px]', isDark ? 'text-emerald-400/60' : 'text-emerald-600/60')}>
                Un agente implementó este ticket automáticamente
              </p>
            </div>
            <Bot className={cn('size-4 shrink-0', isDark ? 'text-emerald-500/60' : 'text-emerald-400')} />
          </div>
        </div>
      )}

      {/* AI Readiness bar (only when no agent running/done banners) */}
      {!isAgentWorking && !doneByAi && (
      ...
      <div className={cn('flex border-b shrink-0', isDark ? 'border-zinc-800' : 'border-gray-200')}>
        {tabs.map(({ k, label, icon }) => {
          const isAiTab = k === 'ai';
          const isAiActive = isAiTab && isAgentWorking;
          const isAiDone = isAiTab && doneByAi && !isAgentWorking;
          return (
            <button
              key={k}
            ...
            >
              {icon} {label}
              {isAiActive && activeTab !== k && (
                <span className={cn('size-1.5 rounded-full animate-pulse', isDark ? 'bg-violet-400' : 'bg-violet-500')} />
              )}
              {isAiDone && activeTab !== k && (
                <span className={cn('size-1.5 rounded-full', isDark ? 'bg-emerald-400' : 'bg-emerald-500')} />
              )}
              ...
              <>
                <div className="space-y-2">
                  <div className="flex items-start gap-2">
                    <p className={cn('text-sm font-semibold flex-1 leading-snug', isDark ? 'text-zinc-200' : 'text-gray-800')}>{ticket.title}</p>
                    <button onClick={() => setEditing(true)} className={cn('size-5 rounded flex items-center justify-center shrink-0 mt-0.5', isDark ? 'hover:bg-zinc-800 text-zinc-500' : 'hover:bg-gray-100 text-gray-400')}>
                    <Pencil className="size-3" />
                  </button>
                  </div>

                  <div className={cn('flex items-center gap-3 text-[9px]', isDark ? 'text-zinc-500' : 'text-gray-400')}>
                    ...
                    {ticket.updatedAt && ticket.updatedAt !== ticket.createdAt && (
                      <span className="flex items-center gap-1">
                        ...
                        {new Date(ticket.updatedAt).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className={cn(
                        ...
                        summarizing
                          ...
                          : isDark ? 'bg-violet-600/20 text-violet-400 hover:bg-violet-600/30 border border-violet-600/20' : 'bg-violet-50 text-violet-600 hover:bg-violet-100 border border-violet-200',
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        'flex items-center gap-1 px-2 py-1 rounded-md text-[9px] font-medium transition-all',
                        isDark ? 'bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 border border-emerald-600/20' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100 border border-emerald-200',
                      )}
                    >
                ...
                {summaryPreview && (
                  <div className={cn('rounded-lg border p-3 space-y-2', isDark ? 'border-violet-800/40 bg-violet-950/20' : 'border-violet-200 bg-violet-50/50')}>
                    ...
                    {summaryTasks.length > 0 && (
                      <div className={cn('rounded-lg border overflow-hidden', isDark ? 'border-zinc-800' : 'border-gray-200')}>
                        <div className={cn(
                          'flex items-center gap-1.5 px-2.5 py-1.5 text-[9px] font-semibold uppercase tracking-wider',
                          isDark ? 'bg-zinc-900 text-emerald-400 border-b border-zinc-800' : 'bg-gray-50 text-emerald-600 border-b border-gray-200',
                        )}>
                          <ListTodo className="size-2.5" />
                          {t('kanbanAiTasksGenerated')} ({summaryTasks.length})
                        </div>
                        <div className="max-h-[150px] overflow-y-auto">
                          {summaryTasks.map((task, idx) => (
                            <div
                              key={idx}
                              className={cn(
                                'flex items-start gap-2 px-2.5 py-1.5 border-b last:border-b-0',
                                isDark ? 'border-zinc-800/60' : 'border-gray-100',
                              )}
                            >
                              <div className={cn(
                                'size-4 rounded flex items-center justify-center shrink-0 mt-0.5 text-[8px] font-bold',
                                isDark ? 'bg-emerald-600/20 text-emerald-400' : 'bg-emerald-100 text-emerald-600',
                              )}>
                                {idx + 1}
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className={cn('text-[10px] font-medium', isDark ? 'text-zinc-200' : 'text-gray-800')}>{task.title}</p>
                                {task.description && (
                                  <p className={cn('text-[9px] mt-0.5 truncate', isDark ? 'text-zinc-500' : 'text-gray-400')}>{task.description}</p>
                                )}
                              </div>
                              <span className={cn(
                                'text-[8px] font-medium px-1.5 py-0.5 rounded-full shrink-0',
                                task.priority === 'high'
                                  ? isDark ? 'bg-red-950/50 text-red-400' : 'bg-red-50 text-red-600'
                                  : task.priority === 'low'
                                    ? isDark ? 'bg-zinc-800 text-zinc-500' : 'bg-gray-100 text-gray-500'
                                    : isDark ? 'bg-amber-950/50 text-amber-400' : 'bg-amber-50 text-amber-600',
                              )}>
                                {task.priority}
                              </span>
                            </div>
                    ...
                    <div className="flex gap-2 pt-2 border-t border-dashed" style={{ borderColor: isDark ? '#3f3f46' : '#e5e7eb' }}>
                      ...
                      >
                        <Save className="size-3" /> {t('kanbanAiSummarizeApply')}{summaryTasks.length > 0 ? ` + ${summaryTasks.length} ${t('kanbanAiTasks')}` : ''}
                      </button>
                      <button
                        onClick={() => {
                          setEditDesc(summaryPreview);
          ...
          <div className="space-y-3">
            {isAgentWorking ? (
              ...
              <>
                ...
                {aiGenerating && studyStep >= 0 && (
                  <AiStepsIndicator steps={AI_STUDY_STEPS} labels={AI_STUDY_LABELS} currentStep={studyStep} done={studyStepsDone} isDark={isDark} />
                )}
            ...
            {/* Sticky bottom: Retry + Apply with Agent */}
            {aiStudy && !isAgentWorking && (
              <div className={cn(
                'sticky bottom-0 z-10 flex items-center gap-2 px-3 py-2.5 -mx-3 -mb-3 mt-2 border-t backdrop-blur-md',
                isDark ? 'bg-zinc-950/90 border-zinc-800' : 'bg-white/90 border-gray-200',
              )}>
                <button
                  onClick={handleAiStudy}
                  className={cn(
                    'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-medium transition-colors',
                    isDark ? 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700 border border-zinc-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 border border-gray-200',
                  )}
                >
                  <RotateCcw className="size-3" /> {t('kanbanRetry')}
                </button>
                ...
                {onCreateAgent && (
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-[10px] font-semibold transition-all shadow-sm',
                      'bg-emerald-600 text-white hover:bg-emerald-500 hover:shadow-emerald-500/20 hover:shadow-md',
                    )}
                  >

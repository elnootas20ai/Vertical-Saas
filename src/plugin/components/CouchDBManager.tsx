...
function DatabaseList({ isDark, t, onOpen }: DatabaseListProps) {
  ...
  const sortedDbs = useMemo(() => {
    ...
    if (activeTag !== null) {
      if (activeTag === '__untagged__') {
        list = list.filter((db) => !meta[db]?.tag);
      } else {
        list = list.filter((db) => meta[db]?.tag === activeTag);
      }
    }
    ...
    return list;
  }, [databases, search, meta, activeTag]);
  ...
  const startEditMeta = (db: string) => {
    ...
    setEditTag(meta[db]?.tag || '');
  };

  const saveEditMeta = () => {
    if (editingMeta) {
      updateMeta(editingMeta, { alias: editAlias.trim() || undefined, description: editDesc.trim() || undefined, tag: editTag.trim() || undefined });
      setEditingMeta(null);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Tag filter tabs */}
      {allTags.length > 0 && (
        <div className={cn('flex items-center gap-1 px-3 py-1.5 border-b shrink-0 overflow-x-auto', isDark ? 'border-zinc-800' : 'border-gray-200')}>
          <button
            onClick={() => setActiveTag(null)}
            className={cn(
              'shrink-0 text-[10px] font-medium px-2.5 py-1 rounded-full transition-all',
              activeTag === null
                ? 'bg-orange-600 text-white shadow-sm'
                : isDark ? 'bg-zinc-800 text-zinc-400 hover:text-zinc-200' : 'bg-gray-100 text-gray-500 hover:text-gray-700',
            )}
          >
            {t('dbTagAll')} ({databases.length})
          </button>
          {allTags.map((tag) => {
            const count = databases.filter(db => meta[db]?.tag === tag).length;
            return (
              <button
                key={tag}
                onClick={() => setActiveTag(activeTag === tag ? null : tag)}
                className={cn(
                  'shrink-0 flex items-center gap-1 text-[10px] font-medium px-2.5 py-1 rounded-full transition-all',
                  activeTag === tag
                    ? 'bg-orange-600 text-white shadow-sm'
                    : isDark ? 'bg-zinc-800 text-zinc-400 hover:text-zinc-200' : 'bg-gray-100 text-gray-500 hover:text-gray-700',
                )}
              >
                ...
                {tag}
                <span className={cn('text-[8px] opacity-70')}>({count})</span>
              </button>
            );
          })}
          <button
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          <TabLoader compact text={t('loading')} />
        ) : sortedDbs.length === 0 ? (
                    ...
                    <div className={cn('flex items-center gap-2 mt-1 text-[9px]', isDark ? 'text-zinc-600' : 'text-gray-400')}>
                      {m.tag && (
                        <span className={cn(
                          ...
                          isDark ? 'bg-orange-950/40 text-orange-400 ring-1 ring-orange-800/30' : 'bg-orange-50 text-orange-600 ring-1 ring-orange-200',
                        )}>
                          <Tag className="size-2" />{m.tag}
                        </span>
                      )}
      ...
      {editingMeta && (
        <>
          ...
          <div className={cn(
            ...
            />
            <label className={cn('text-[10px] font-medium mb-1 block', isDark ? 'text-zinc-400' : 'text-gray-500')}>
              <Tag className="size-3 inline mr-1" />{t('dbTag')}
            </label>
            <div className="flex items-center gap-2 mb-3">
              <input
                className={cn('flex-1 rounded-lg px-3 py-1.5 text-xs outline-none border', isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-200' : 'bg-gray-50 border-gray-200 text-gray-900')}
                value={editTag}
                onChange={(e) => setEditTag(e.target.value)}
                placeholder={t('dbTagPlaceholder')}
              />
              {allTags.length > 0 && (
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        editTag === tag
                          ...
                          : isDark ? 'bg-zinc-800 text-zinc-500 hover:text-zinc-300' : 'bg-gray-100 text-gray-500 hover:text-gray-700',
                      )}
                    >
                      {tag}
                    </button>

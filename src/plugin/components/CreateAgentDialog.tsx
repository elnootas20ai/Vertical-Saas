...
export function CreateAgentDialog({ open, onClose, onCreate }: Props) {
  ...
  const [type, setType] = useState<AgentType>('cursor');
  const [cwd, setCwd] = useState('');
  const [model, setModel] = useState('claude-4.6-sonnet-medium-thinking');
  ...
  const handleCreate = () => {
    ...
    setName('');
    setCwd('');
    setModel('claude-4.6-sonnet-medium-thinking');
    setPrompt('');
    setType('cursor');
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-stretch">
      ...
      <div
        className={cn(
          'relative flex flex-col w-full max-w-lg mx-auto overflow-hidden animate-in fade-in zoom-in-95 duration-200 border-x',

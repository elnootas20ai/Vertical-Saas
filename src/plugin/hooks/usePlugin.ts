...
export function usePlugin() {
  ...
  const stopAgent = useCallback(async () => {
    if (!activeAgentId) return;
    try {
      await agentApi.stopChat(activeAgentId);
      setIsStreaming(false);
      setIsThinking(false);
      setAgents((prev) => prev.map((a) => a.id === activeAgentId ? { ...a, status: 'idle' } : a));
    } catch { /* best effort */ }
  }, [activeAgentId]);

  const forceResetAgent = useCallback(async () => {
    if (!activeAgentId) return;
    const savedId = activeAgentId;
    try { await agentApi.stopChat(savedId); } catch { /* */ }
    setIsStreaming(false);
    setIsThinking(false);
    setError(null);
    setAgents((prev) => prev.map((a) => a.id === savedId ? { ...a, status: 'idle' } : a));
    if (chatCleanupRef.current) {
      chatCleanupRef.current();
      chatCleanupRef.current = null;
    }
    setActiveAgentId(null);
    setTimeout(() => setActiveAgentId(savedId), 100);
  }, [activeAgentId]);
  ...
  useEffect(() => {
    ...
    const cleanup = agentApi.watchChat(activeAgentId, (event) => {
      if (event.type === 'heartbeat') {
        setLastHeartbeat(event.ts || Date.now());
        if (event.agentStatus) {
          setAgents((prev) => prev.map((a) => a.id === activeAgentId ? { ...a, status: event.agentStatus!.status as Agent['status'] } : a));
          if (!event.agentStatus.isProcessing) {
            setIsStreaming(false);
            setIsThinking(false);
          }
        }
      ...
      if (event.type === 'thinking_start' || event.type === 'message_start') {
        ...
        setAgents((prev) => prev.map((a) => a.id === activeAgentId ? { ...a, status: event.type === 'message_error' ? 'error' : 'idle', updatedAt: new Date().toISOString() } : a));
      }

      if (event.type === 'init') {
        if (event.messages) setMessages(event.messages);
        if (event.agentStatus) {
          const st = event.agentStatus as { status: string; isProcessing: boolean };
          if (st.isProcessing) {
            setIsStreaming(true);
            setAgents((prev) => prev.map((a) => a.id === activeAgentId ? { ...a, status: 'running' } : a));
          }
        ...
        setIsStreaming(false);
        setIsThinking(false);
        const errorDetail = event.error || 'Error desconocido del agente';
        if (event.message) {
          setMessages((prev) =>
            prev.map((m) => (m.id === event.message!.id
              ? { ...event.message!, thinking: m.thinking, isError: true, errorDetail }
              : m)),
          );
        }
  ...
  useEffect(() => {
    ...
    const cleanup = agentApi.watchQueue(activeAgentId, (event) => {
      ...
      if (event.error) setError(`Cola: ${event.error}`);
    });

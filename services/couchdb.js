...
function sanitizeIntegrationEntry(entry) {
  ...
  return {
    ...
    storeSelectionRequired: Boolean(entry.storeSelectionRequired),
...
export function buildWebOrderDocument(businessId, data = {}, existing = null) {
  ...
  return {
    ...
    orderNumber,
    business_id: businessId,
    sourceChannel: String(data.sourceChannel || existing?.sourceChannel || 'public_web'),
    targetKind: String(data.targetKind || existing?.targetKind || 'delivery_ops'),
    reviewStatus: String(data.reviewStatus || existing?.reviewStatus || 'pending'),
    linkedDiningOrderId: String(data.linkedDiningOrderId || existing?.linkedDiningOrderId || ''),
    linkedComandaId: String(data.linkedComandaId || existing?.linkedComandaId || ''),
    linkedDeliveryOrderId: String(data.linkedDeliveryOrderId || existing?.linkedDeliveryOrderId || ''),
    reviewedAt: String(data.reviewedAt || existing?.reviewedAt || ''),
    reviewedBy: String(data.reviewedBy || existing?.reviewedBy || ''),
    rejectionReason: String(data.rejectionReason || existing?.rejectionReason || ''),
    salesPointId: String(data.salesPointId || existing?.salesPointId || '').trim(),
...
export function sanitizeWebOrder(doc) {
  ...
  return {
    ...
    orderNumber: doc.orderNumber || '',
    business_id: doc.business_id || '',
    sourceChannel: doc.sourceChannel || (doc.tableId ? 'restaurant_qr' : 'public_web'),
    targetKind: doc.targetKind || (doc.tableId ? 'restaurant_table' : 'delivery_ops'),
    reviewStatus: doc.reviewStatus || (doc.status === 'pending' ? 'pending' : 'accepted'),
    linkedDiningOrderId: doc.linkedDiningOrderId || '',
    linkedComandaId: doc.linkedComandaId || '',

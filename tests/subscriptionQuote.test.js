...
describe('subscriptionQuote', () => {
  ...
  it('PRO + 1 PDV + 3 trab. mensuales', () => {
    const q = quoteSubscription({
      planId: 'pro',
      billingMode: 'monthly',
      metrics: {
        userCount: 15, // pro incluye 12 → +3
        locationCount: 3, // pro incluye 2 → +1
        businessCount: 1,
        commercialBrandCount: 0,
      },
    });
    // 349 + 49 + 3*5 = 413
    expect(q.extras.extraPdv).toBe(1);
    expect(q.extras.extraWorkers).toBe(3);
    expect(q.amountDueEuros).toBe(413);
  });
  ...
  it('PRO + extras en anual usa la misma lista ×12×0,8', () => {
    ...
    expect(q.listMonthlyEuros).toBe(413);
    expect(q.amountDueEuros).toBe(3964.8); // 413*12*0.8
  });
  ...
  it('prioriza métricas del onboarding sobre slots vacíos en subscription', () => {
    ...
    expect(q.listMonthlyEuros).toBe(413);
    expect(q.amountDueEuros).toBe(3964.8);
  });
});
